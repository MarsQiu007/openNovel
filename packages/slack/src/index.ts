...
import { createOpenNovel, type ToolPart } from "@opennovel-ai/sdk"
...
console.log("🚀 Starting opennovel server...")
const opennovel = await createOpenNovel({
  port: 0,
})
console.log("✅ OpenNovel server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await opennovel.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new opennovel session...")
    const { client, server } = opennovel
    ...
    console.log("✅ Created opennovel session:", createResult.data.id)
  ...
  console.log("📝 Sending to opennovel:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OpenNovel response:", JSON.stringify(result, null, 2))
