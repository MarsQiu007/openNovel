...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve opennovel route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "opennovel")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "opennovel")).toBe("openai")
    expect(statProvider("big-pickle", "", "opennovel")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "opennovel"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep opennovel as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "opennovel"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "opennovel"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep opennovel or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "opennovel"), country: "US" })).toMatchObject([
