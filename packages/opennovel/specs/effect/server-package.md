...
Practical reference for a future `packages/server` split after the opennovel
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/opennovel`.
...
Until enough shared service code lives outside `packages/opennovel`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/opennovel`

It should not import `packages/opennovel` services while `packages/opennovel`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on opennovel-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/opennovel` implementation details.
