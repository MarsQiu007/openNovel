...
- Author package entrypoint is `@opennovel-ai/plugin/tui`.
...
```json
{
  "$schema": "https://opennovel.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/opennovel-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@opennovel-ai/plugin/tui`.
- `@opennovel-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@opennovel-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenNovel version.
...
- `opennovel plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `opennovel plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.opennovel` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.opennovel`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.opennovel`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.opennovel` dir or `OPENNOVEL_CONFIG_DIR`, OpenNovel installs `@opennovel-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@opennovel-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenNovel's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OpenNovel registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, OpenNovel pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@opennovel-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"opennovel"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `opennovel.default`.
...
- Local plugins persist installed themes under the local `.opennovel/themes` area near the plugin config source.
...
- `--pure` / `OPENNOVEL_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.opennovel/plugins/tui-smoke.tsx`
- Local vim plugin: `.opennovel/plugins/tui-vim.tsx`
- Local smoke config: `.opennovel/tui.json`
- Local smoke theme: `.opennovel/plugins/smoke-theme.json`
