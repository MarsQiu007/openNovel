...
Delete `packages/opennovel/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/opennovel/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/opennovel/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/opennovel`
- TUI plugin loader tests in `packages/opennovel` if runtime plugin API wiring changed
