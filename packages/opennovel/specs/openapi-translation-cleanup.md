...
Trim `packages/opennovel/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/opennovel/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/opennovel`.
- `[x]` `bun typecheck` from `packages/opennovel`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/opennovel/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/opennovel`.
- `[x]` `bun dev generate > /tmp/opennovel-openapi.json` from `packages/opennovel`.
...
- Edit `packages/opennovel/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opennovel-openapi.json` from `packages/opennovel`.
...
- Check source schemas in files like `packages/opennovel/src/session/schema.ts`, `packages/opennovel/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/opennovel/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/opennovel/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
