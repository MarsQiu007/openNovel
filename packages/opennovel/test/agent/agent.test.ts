...
it.instance("plan agent denies edits except .opennovel/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".opennovel/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".opennovel", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENNOVEL_TEST_HOME
      process.env.OPENNOVEL_TEST_HOME = test.directory
