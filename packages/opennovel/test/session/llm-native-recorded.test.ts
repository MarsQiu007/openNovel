...
import { SessionV1 } from "@opennovel-ai/core/v1/session"
import { ModelsDev } from "@opennovel-ai/core/models-dev"
import { HttpRecorder } from "@opennovel-ai/http-recorder"
import { HttpRecorderInternal } from "@opennovel-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@opennovel-ai/llm"
import { RequestExecutor } from "@opennovel-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.opennovel.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENNOVEL_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["opennovel", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENNOVEL_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OPENNOVEL_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["opennovel", "native", "oauth", "tool-loop"],
  ...
  {
    id: "opennovel-proxy",
    name: "OpenNovel proxy",
    providerID: ProviderV2.ID.opennovel,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opennovel", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OPENNOVEL_RECORD_CONSOLE_TOKEN && process.env.OPENNOVEL_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opennovel,
        name: "OpenNovel Zen",
        env: ["OPENNOVEL_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OPENNOVEL_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENNOVEL_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENNOVEL_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opennovel", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENNOVEL_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OPENNOVEL_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OPENNOVEL_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OPENNOVEL_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the opennovel LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "opennovel.json"),
      JSON.stringify({ $schema: "https://opennovel.ai/config.json", ...scenario.config(model) }),
    ),
  )
