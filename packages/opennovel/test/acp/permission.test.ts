...
import type { Event, OpenNovelClient } from "@opennovel-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<OpenNovelClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "opennovel-acp-permission-"))
