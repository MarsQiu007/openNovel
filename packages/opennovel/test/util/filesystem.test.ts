...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "opennovel.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "opennovel.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opennovel.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opennovel.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["opennovel.json", "opennovel.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "opennovel.json"),
        path.join(project, "opennovel.jsonc"),
      ])
    })
  })
