...
const noAuthLayer = ServerAuth.Config.configLayer({ password: Option.none(), username: "opennovel" })
const secretLayer = ServerAuth.Config.configLayer({ password: Option.some("secret"), username: "opennovel" })
...
describe("HttpApi authorization middleware", () => {
  ...
  itSecret.live("requires configured password for basic auth", () =>
    Effect.gen(function* () {
      const [missing, badPassword, good] = yield* Effect.all(
        [
          getProbe(),
          getProbe({ authorization: basic("opennovel", "wrong") }),
          getProbe({ authorization: basic("opennovel", "secret") }),
        ],
  ...
  itSecret.live("accepts auth token query credentials", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/probe?auth_token=${encodeURIComponent(token("opennovel", "secret"))}`)
  ...
  itSecret.live("prefers auth token query credentials over basic auth", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get(
        `/probe?auth_token=${encodeURIComponent(token("opennovel", "secret"))}`,
      ).pipe(HttpClientRequest.setHeader("authorization", basic("opennovel", "wrong")), HttpClient.execute)
  ...
  itSecret.live("preserves handler errors when basic auth succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get("/missing").pipe(
        HttpClientRequest.setHeader("authorization", basic("opennovel", "secret")),
        HttpClient.execute,
      )
  ...
  itSecret.live("preserves handler errors when auth token query succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/missing?auth_token=${encodeURIComponent(token("opennovel", "secret"))}`)
