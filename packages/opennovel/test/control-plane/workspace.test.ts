...
import { EventSequenceTable } from "@opennovel-ai/core/event/sql"
...
import { WorkspaceV2 } from "@opennovel-ai/core/workspace"
import { WorkspaceTable } from "@opennovel-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  OPENNOVEL_AUTH_CONTENT: process.env.OPENNOVEL_AUTH_CONTENT,
  OPENNOVEL_EXPERIMENTAL_WORKSPACES: process.env.OPENNOVEL_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OPENNOVEL_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@opennovel.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENNOVEL_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=opennovel-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENNOVEL_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENNOVEL_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENNOVEL_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=opennovel-test")
