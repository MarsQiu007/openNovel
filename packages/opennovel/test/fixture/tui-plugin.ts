...
import type { HostPluginApi } from "@opennovel-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createOpenNovelClient({
  ...
  let selected = opts.theme?.selected ?? "opennovel"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "opennovel.default",
