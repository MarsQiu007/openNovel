...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ opennovel }) =>
    Effect.gen(function* () {
      const result = yield* opennovel.spawn(["--replay-limit", "10"])

      opennovel.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ opennovel }) =>
    Effect.gen(function* () {
      const result = yield* opennovel.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ opennovel }) =>
    Effect.gen(function* () {
      const result = yield* opennovel.spawn(["--mini", "--port", "4096"])
