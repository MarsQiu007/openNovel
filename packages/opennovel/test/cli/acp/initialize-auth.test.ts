...
describe("opennovel acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ opennovel }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ opennovel }))
        ...
        expect(initialized.agentInfo?.name).toBe("OpenNovel")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ opennovel })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("opennovel-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "opennovel-login" })).toMatchObject(
          {
            result: {},
          },
        )
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OPENNOVEL_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("opennovel-login")
