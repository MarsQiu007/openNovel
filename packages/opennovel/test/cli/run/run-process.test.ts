// Subprocess integration tests for `opennovel run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `opennovel.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENNOVEL_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("opennovel run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("say hi")
        opennovel.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* opennovel.run("think", { extraArgs: ["--thinking"] })
        opennovel.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* opennovel.run("think again")
        opennovel.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("say hi", { format: "json" })
        opennovel.expectExit(result, 0)

        const events = opennovel.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const result = yield* opennovel.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opennovel.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* opennovel.run("request permission", { permission: { bash: "ask" } })
        opennovel.expectExit(denied, 0)
        ...
        const allowed = yield* opennovel.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        opennovel.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* opennovel.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        opennovel.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, opennovel }) =>
      Effect.gen(function* () {
        ...
        const server = yield* opennovel.serve()

        const result = yield* opennovel.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, opennovel }) =>
      Effect.gen(function* () {
        const result = yield* opennovel.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, opennovel }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* opennovel.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
