...
import { OpenNovelClient, type GlobalEvent } from "@opennovel-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OpenNovelClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OpenNovelClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OpenNovelClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OpenNovelClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OpenNovelClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://opennovel.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OpenNovelClient["event"]["subscribe"]
    globalEvent?: OpenNovelClient["global"]["event"]
    promptAsync?: OpenNovelClient["session"]["promptAsync"]
    status?: OpenNovelClient["session"]["status"]
    messages?: OpenNovelClient["session"]["messages"]
    children?: OpenNovelClient["session"]["children"]
    permissions?: OpenNovelClient["permission"]["list"]
    questions?: OpenNovelClient["question"]["list"]
  } = {},
) {
  const client = new OpenNovelClient()

  const subscribe: OpenNovelClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OpenNovelClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OpenNovelClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OpenNovelClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OpenNovelClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OpenNovelClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OpenNovelClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OpenNovelClient["question"]["list"] = input.questions ?? (() => ok([]))
