...
describe("opennovel read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENNOVEL_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["mcp", "list"])
        opennovel.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["providers", "list"])
        opennovel.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["models"])
        opennovel.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["agent", "list"])
        opennovel.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENNOVEL_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["session", "list"])
        opennovel.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["stats"])
        opennovel.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENNOVEL_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ opennovel }) =>
      Effect.gen(function* () {
        const r = yield* opennovel.spawn(["db", "path"])
        opennovel.expectExit(r, 0, "db path")
