// Subprocess test harness for the opennovel CLI. Spawns the real binary against
...
//
// Configuration flows through opennovel's built-in test affordances:
//   - OPENNOVEL_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENNOVEL_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENNOVEL_DISABLE_PROJECT_CONFIG : skip walking up for opennovel.json
//   - OPENNOVEL_PURE                : skip external plugin discovery + install
//   - OPENNOVEL_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `opennovel.run` is fully wired. The shape supports adding more
// builders (`opennovel.serve(opts)`, `opennovel.acp(opts)`, `opennovel.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpenNovelCli.
...
import { AppProcess } from "@opennovel-ai/core/process"
...
const opennovelRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(opennovelRoot, "src/index.ts")
// Windows does not reliably resolve bare executable names through Bun.spawn.
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENNOVEL_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENNOVEL_CONFIG_CONTENT: configJson,
    OPENNOVEL_DISABLE_PROJECT_CONFIG: "1",
    OPENNOVEL_PURE: "1",
    OPENNOVEL_DISABLE_AUTOUPDATE: "1",
    OPENNOVEL_DISABLE_AUTOCOMPACT: "1",
    OPENNOVEL_DISABLE_MODELS_FETCH: "1",
    OPENNOVEL_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `opennovel run`. New flags should
...
// `opennovel serve` is a long-lived process — it never exits on its own.
...
// `opennovel acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpenNovelCli = {
  ...
  // Spawn `opennovel serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `opennovel acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly opennovel: OpenNovelCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("opennovel.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OPENNOVEL_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("opennovel.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
      const options = runOpts(opts)
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn([bunExecutable, "run", "--conditions=browser", cliEntry, ...runArgs(message, opts)], {
            cwd: home,
    ...
    const serve = Effect.fn("opennovel.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "opennovel server listening on http://<host>:<port>"
      const readyRe = /(?:listening on|服务器监听于)\s+(http:\/\/([^\s:]+):(\d+))/
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `opennovel serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("opennovel.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const opennovel: OpenNovelCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, opennovel })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `opennovel.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "opennovel") {
...
// every call site. Use this for any test that needs the opennovel CLI fixture.
//
...
// resources (e.g. `opennovel.serve`) without an extra `Effect.scoped` wrapper —
