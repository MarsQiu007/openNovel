...
import { FrontmatterError } from "@opennovel-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@opennovel-ai/core/util/glob"
...
const OPENNOVEL_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with opennovel. The model's intuition for what an
// opennovel.json should look like is often wrong, and opennovel hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch opennovel own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_OPENNOVEL_SKILL_NAME = "customize-opennovel"
const CUSTOMIZE_OPENNOVEL_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating opennovel's own configuration: opennovel.json, opennovel.jsonc, files under .opennovel/, or files under ~/.config/opennovel/. Also use when creating or fixing opennovel agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring opennovel itself."
const CUSTOMIZE_OPENNOVEL_SKILL_BODY = SkillPlugin.CustomizeOpenNovelContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, OPENNOVEL_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@opennovel/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_OPENNOVEL_SKILL_NAME] = {
          name: CUSTOMIZE_OPENNOVEL_SKILL_NAME,
          description: CUSTOMIZE_OPENNOVEL_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_OPENNOVEL_SKILL_BODY,
        }
