...
import { isRecord } from "@opennovel-ai/tui/util/record"
...
import { TuiKeybind } from "@opennovel-ai/tui/config/keybind"
...
import { TuiConfig } from "@opennovel-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@opennovel/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old opennovel.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.opennovel` / `.opennovel`
  // folders between cwd and home, and OPENNOVEL_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPENNOVEL_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENNOVEL_TUI_CONFIG override, if set.
  if (Flag.OPENNOVEL_TUI_CONFIG) {
    const configFile = Flag.OPENNOVEL_TUI_CONFIG
  ...
  // 4. `.opennovel` / `.opennovel` directories (and OPENNOVEL_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".opennovel") || dir.endsWith(".opennovel") || dir === Flag.OPENNOVEL_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".opennovel") && !dir.endsWith(".opennovel") && dir !== Flag.OPENNOVEL_CONFIG_DIR) continue
