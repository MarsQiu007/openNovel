...
const TUI_SCHEMA_URL = "https://opennovel.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from opennovel.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const opennovelCfgs = await opennovelFiles(input)
  for (const file of opennovelCfgs) {
...
async function opennovelFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opennovel"),
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opennovel"),
    ...(await Filesystem.findUp(["opennovel.json", "opennovel.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opennovel"))
    files.push(...ConfigPaths.fileInDirectory(dir, "opennovel"))
  }
  if (Flag.OPENNOVEL_CONFIG) files.push(Flag.OPENNOVEL_CONFIG)
