...
import { IdeEvent } from "@opennovel-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["OPENNOVEL_CALLER"] === "vscode" || process.env["OPENNOVEL_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.opennovel"], {
    nothrow: true,
  })
