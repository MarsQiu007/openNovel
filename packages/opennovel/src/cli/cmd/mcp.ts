...
import { ConfigMCPV1 } from "@opennovel-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("使用以下命令添加服务器: opennovel mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("远程 MCP 服务器默认支持 OAuth。在 opennovel.json 中添加远程服务器:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .opennovel/ subdirectory too)
  const candidates = [path.join(baseDir, "opennovel.json"), path.join(baseDir, "opennovel.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".opennovel", "opennovel.json"),
      path.join(baseDir, ".opennovel", "opennovel.jsonc"),
    )
  }
  ...
  // Default to opennovel.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "例如, opennovel x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "opennovel-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
