...
import type { Hooks } from "@opennovel-ai/plugin"
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/opennovel`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(`这仅为 ${provider} 存储凭据 —— 您需要在 opennovel.json 中配置它，请查看文档获取示例。`)
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "通过 opennovel.json 选项 (profile, region, endpoint) 或\n" +
          "AWS 环境变量 (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE) 进行配置。",
      )
    }

    if (provider === "opennovel") {
      yield* Prompt.log.info("Create an api key at https://opennovel.ai/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://opennovel.ai/docs/providers/#cloudflare-ai-gateway",
      )
    }
