...
export const UninstallCommand = {
  command: "uninstall",
  describe: "卸载 opennovel 并移除所有相关文件",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenNovel")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opennovel-ai",
      pnpm: "pnpm uninstall -g opennovel-ai",
      bun: "bun remove -g opennovel-ai",
      yarn: "yarn global remove opennovel-ai",
      brew: "brew uninstall opennovel",
      choco: "choco uninstall opennovel",
      scoop: "scoop uninstall opennovel",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "opennovel", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenNovel!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opennovel") || content.includes(".opennovel/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opennovel/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opennovel/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opennovel"))
    ) {
      continue
    }
