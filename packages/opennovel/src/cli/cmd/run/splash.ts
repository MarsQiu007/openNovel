// 直连交互模式滚动历史的进入和退出横幅。
//
// 渲染完整的 OpenNovel 进入徽标和紧凑的 [O] 退出徽章，
// 以及会话元数据和恢复命令。这些是滚动历史快照，一旦提交即成为不可变的终端历史。
//
// 两种变体都使用基于单元格的渲染器。cells() 将源模板中的每个字符分类为文本、全块、半块混合或半块顶部，
// draw() 使用主题中的前景/背景阴影颜色进行渲染。
import {
  BoxRenderable,
  type ColorInput,
  TextAttributes,
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "OpenNovel", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `opennovel --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
