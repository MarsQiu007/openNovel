...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && (provider.id === "opennovel" || provider.id === "opennovel")
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider =
          Number(a.providerID !== "opennovel" && a.providerID !== "opennovel") -
          Number(b.providerID !== "opennovel" && b.providerID !== "opennovel")
        if (provider !== 0) {
          return provider
        }
