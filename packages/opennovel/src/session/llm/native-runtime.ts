...
import type { LLMClientShape } from "@opennovel-ai/llm/route"
...
function statusWithFetch(
  ...
  if (
    ...
    !providerID.startsWith("opennovel") &&
    !providerID.startsWith("opennovel")
  )
    return { type: "unsupported", reason: "provider is not openai, opennovel, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @opennovel-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains opennovel-owned. The native runtime only adapts
      // the @opennovel-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
