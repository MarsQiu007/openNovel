...
import type { AssistantMessage, Message, OpenNovelClient, SessionMessageResponse } from "@opennovel-ai/sdk/v2"
...
export const AuthMethodID = "opennovel-login"
...
export class Service extends Context.Service<Service, Interface>()("@opennovel/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `opennovel auth login` in the terminal",
      name: "Login with OpenNovel",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OpenNovelClient) {
...
function makeUsageService(sdk: OpenNovelClient) {
...
async function loadDirectorySnapshot(sdk: OpenNovelClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, opennovel provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const opennovelProvider = providers[ProviderV2.ID.make("opennovel")]
  const opennovelModel = opennovelProvider ? Provider.sort(Object.values(opennovelProvider.models))[0] : undefined
  if (opennovelProvider && opennovelModel) return { providerID: opennovelProvider.id, modelID: opennovelModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "OpenNovel prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenNovel service failure", service })
}
