...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@opennovel-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@opennovel-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@opennovel-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@opennovel-ai/tui/context/theme"
import { Global } from "@opennovel-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@opennovel-ai/core/util/flock"
import { Flag } from "@opennovel-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@opennovel-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@opennovel-ai/core/v1/config/plugin"
import { createCommandShim } from "@opennovel-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@opennovel-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".opennovel"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".opennovel", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".opennovel", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPENNOVEL_PURE ? [] : pluginOrigins
    if (Flag.OPENNOVEL_PURE && pluginOrigins.length) {
    }
