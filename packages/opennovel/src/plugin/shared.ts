...
export async function checkPluginCompatibility(target: string, opennovelVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(opennovelVersion) || semver.major(opennovelVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.opennovel ?? engines.opennovel
  if (typeof range !== "string") return
  if (!semver.satisfies(opennovelVersion, range)) {
    throw new Error(`Plugin requires opennovel ${range} but running ${opennovelVersion}`)
  }
}
