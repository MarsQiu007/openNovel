...
let releaseOpenNovelResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOpenNovelVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectOpenNovelVersion("1.14.35", "1.16.2")).toThrow(
    "OpenNovel update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating OpenNovel", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.opennovel/bin/opennovel",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, opennovelChecks: {} })
})
...
test("ignores stale background OpenNovel checks after removing a WSL server", async () => {
  persistedServers = []
  releaseOpenNovelResolve = undefined
  ...
  await waitFor(() => !!releaseOpenNovelResolve)
  await controller.removeServer("wsl:Debian")
  releaseOpenNovelResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().opennovelChecks).toEqual({})
})

test("ignores stale startup OpenNovel checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking OpenNovel", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const opennovel: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpenNovel: async (distro) => {
      opennovel.push(distro)
      return "/home/me/.opennovel/bin/opennovel"
    },
  })
  ...
  expect(opennovel).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(opennovel).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().opennovelChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check OpenNovel in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opennovel: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(opennovel).toEqual(["Debian"])
  expect(Object.keys(controller.getState().opennovelChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveOpenNovel: async () => {
      await new Promise<void>((resolve) => {
        releaseOpenNovelResolve = resolve
      })
      return "/home/me/.opennovel/bin/opennovel"
    },
  }
