import type { WslDistroProbe, WslOpenNovelCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  opennovelChecks: Record<string, WslOpenNovelCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextOpenNovelChecks = { ...opennovelChecks }
  delete nextDistroProbes[distro]
  delete nextOpenNovelChecks[distro]
  return { distroProbes: nextDistroProbes, opennovelChecks: nextOpenNovelChecks }
}
