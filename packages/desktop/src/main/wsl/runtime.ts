...
export async function installWslOpenNovel(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      [
        "bash",
        "-lc",
        `curl -fsSL https://raw.githubusercontent.com/MarsQiu007/openNovel/dev/install | bash -s -- --version ${shellEscape(version)}`,
      ],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
...
export async function resolveWslOpenNovel(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.opennovel/bin/opennovel" ]; then printf "%s\\n" "$HOME/.opennovel/bin/opennovel"; fi',
        distro,
        opts,
      )
    ).stdout,
