...
import { expectOpenNovelVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOpenNovel?: typeof resolveWslOpenNovel
...
export function createWslServersController(
  ...
  const setOpenNovelCheck = (distro: string, check: WslOpenNovelCheck) => {
    setState({
      opennovelChecks: {
        ...state.opennovelChecks,
        [distro]: check,
      },
    })
  }

  const checkOpenNovel = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOpenNovel ?? resolveWslOpenNovel)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return opennovelCheck(distro, resolved, version, appVersion)
  }

  const refreshOpenNovelCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOpenNovelCheck(distro, await checkOpenNovel(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const opennovelChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.opennovelChecks[distro])
        .map(async (distro) => [distro, await checkOpenNovel(distro, opts)] as const),
    )
    if (opennovelChecks.length) {
      setState({ opennovelChecks: { ...state.opennovelChecks, ...Object.fromEntries(opennovelChecks) } })
    }
  }
  ...
  const refreshOpenNovelCheckBackground = (id: string, distro: string) => {
    void checkOpenNovel(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOpenNovelCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl opennovel check failed", { id, distro, message })
      })
  }

  const refreshOpenNovelChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOpenNovel(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOpenNovelCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl opennovel check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOpenNovelCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installOpenNovel(name: string) {
      await runJob({ kind: "install-opennovel", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpenNovel(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "OpenNovel installation failed")
        }
        await refreshOpenNovelCheck(name, { signal: abort.signal })
        expectOpenNovelVersion(state.opennovelChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.opennovelChecks, distro) : {}),
      })
    },
...
function opennovelCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "opennovel is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "opennovel is installed but could not run",
    }
  }
  return {
    distro,
