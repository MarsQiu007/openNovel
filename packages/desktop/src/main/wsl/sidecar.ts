...
import { type WslCommandLine, resolveWslOpenNovel, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const opennovel = await resolveWslOpenNovel(distro)
  if (!opennovel) throw new Error(`OpenNovel is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "opennovel"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OPENNOVEL_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPENNOVEL_CLIENT=desktop",
    `export OPENNOVEL_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPENNOVEL_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(opennovel)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
