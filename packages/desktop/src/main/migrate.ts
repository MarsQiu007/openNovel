...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.opennovel.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `opennovel.settings.dat` is special: it maps to the `opennovel.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"opennovel.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opennovel.settings.dat → the electron settings store ("opennovel.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "opennovel.global.dat").
  const storeName = filename === "opennovel.settings.dat" ? "opennovel.settings" : filename
