...
import {
  ...
  useCommand,
  useWslServers,
} from "@opennovel-ai/app"
import type { UpdaterState } from "@opennovel-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@opennovel-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENNOVEL_CHANNEL === "prod" &&
...
const deepLinkEvent = "opennovel:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__OPENNOVEL__ ??= {}
  const pending = window.__OPENNOVEL__.deepLinks ?? []
  window.__OPENNOVEL__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `opennovel.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://opennovel.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("opennovel.global.dat").getItem("language")
