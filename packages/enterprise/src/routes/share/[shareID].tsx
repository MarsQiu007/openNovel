import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@opennovel-ai/sdk/v2"
import { SessionTurn } from "@opennovel-ai/session-ui/session-turn"
import { SessionReview } from "@opennovel-ai/session-ui/session-review"
import { DataProvider } from "@opennovel-ai/session-ui/context"
import { FileComponentProvider } from "@opennovel-ai/ui/context/file"
import { WorkerPoolProvider } from "@opennovel-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@opennovel-ai/ui/logo"
import { IconButton } from "@opennovel-ai/ui/icon-button"
import { ProviderIcon } from "@opennovel-ai/ui/provider-icon"
import { iife } from "@opennovel-ai/core/util/iife"
import { Binary } from "@opennovel-ai/core/util/binary"
import { NamedError } from "@opennovel-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@opennovel-ai/ui/tabs"
import { MessageNav } from "@opennovel-ai/session-ui/message-nav"
import { FileSSR } from "@opennovel-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@opennovel-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/opennovel-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | OpenNovel</Title>
              </Show>
              <Meta name="description" content="opennovel - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/MarsQiu007/openNovel"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://opennovel.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
