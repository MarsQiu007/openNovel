...
import { and, Database, eq, isNull, lt, or, sql } from "@opennovel-ai/console-core/drizzle/index.js"
import { KeyTable } from "@opennovel-ai/console-core/schema/key.sql.js"
import {
  BillingTable,
  LiteTable,
  SubscriptionTable,
  UsageTable,
} from "@opennovel-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@opennovel-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@opennovel-ai/console-core/util/date.js"
import { Identifier } from "@opennovel-ai/console-core/identifier.js"
import { Billing } from "@opennovel-ai/console-core/billing.js"
import { Actor } from "@opennovel-ai/console-core/actor.js"
import { WorkspaceTable } from "@opennovel-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@opennovel-ai/console-core/model.js"
import { Subscription } from "@opennovel-ai/console-core/subscription.js"
import { BlackData } from "@opennovel-ai/console-core/black.js"
import { UserTable } from "@opennovel-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@opennovel-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@opennovel-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@opennovel-ai/console-core/lite.js"
...
import { Workspace } from "@opennovel-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-opennovel-session") ?? ""
    const requestId = input.request.headers.get("x-opennovel-request") ?? ""
    const ocClient = input.request.headers.get("x-opennovel-client") ?? ""
    const projectId = input.request.headers.get("x-opennovel-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://opennovel.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-opennovel-request")
            headers.delete("x-opennovel-session")
            headers.delete("x-opennovel-project")
            headers.delete("x-opennovel-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-opennovel-endpoint-id")
        const resEndpointModelId = res.headers.get("x-opennovel-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://opennovel.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://opennovel.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://opennovel.ai/workspace/${authInfo.workspaceID}/members`
