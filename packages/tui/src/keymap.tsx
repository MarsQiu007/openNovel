...
export const OPENNOVEL_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENNOVEL_MODE_KEY = "opennovel.mode"

export const OpenNovelKeymapProvider = KeymapProvider
export const useOpenNovelKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpenNovelModeStack = ReturnType<typeof createOpenNovelModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpenNovelModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpenNovelModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENNOVEL_MODE_KEY, OPENNOVEL_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENNOVEL_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENNOVEL_MODE_KEY, stack.at(-1)?.mode ?? OPENNOVEL_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENNOVEL_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENNOVEL_MODE_KEY, undefined)
...
export function useOpenNovelModeStack() {
  return getOpenNovelModeStack(useOpenNovelKeymap())
}

export function getOpenNovelModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("OpenNovel mode stack is not registered for this keymap")
  return value
}
...
export function registerOpenNovelKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOpenNovelModeStack(keymap)
