import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@opennovel-ai/plugin/tui"
...
import type { useOpenNovelKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOpenNovelKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpenNovelModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpenNovelModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
