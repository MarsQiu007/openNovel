import type { TuiPluginApi } from "@opennovel-ai/plugin/tui"
...
const NO_MODELS_TIP = "运行 {highlight}/connect{/highlight} 添加 AI 提供商并开始写作"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("opennovel.status"),
...
const TIPS: Tip[] = [
  ...
  "运行 {highlight}/share{/highlight} 创建公开的 opennovel.ai 链接",
  ...
  "先创建一部小说，再使用 {highlight}writer{/highlight} 工作流开始自动续写",
  (shortcuts) => {
    const items = [
  ...
  "Create {highlight}opennovel.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/opennovel/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.opennovel/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.opennovel/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.opennovel/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.opennovel/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "创建插件以防止 OpenNovel 读取敏感文件",
  "使用 {highlight}opennovel run{/highlight} 进行非交互式脚本执行",
  "使用 {highlight}opennovel --continue{/highlight} 恢复上一个会话",
  "使用 {highlight}opennovel run -f file.ts{/highlight} 通过 CLI 附加文件",
  "使用 {highlight}--format json{/highlight} 在脚本中获取机器可读的输出",
  "运行 {highlight}opennovel serve{/highlight} 以无头模式访问 OpenNovel API",
  "使用 {highlight}opennovel run --attach{/highlight} 连接到正在运行的服务器",
  "运行 {highlight}opennovel upgrade{/highlight} 更新到最新版本",
  "运行 {highlight}opennovel auth list{/highlight} 查看所有已配置的提供商",
  "运行 {highlight}opennovel agent create{/highlight} 进行引导式智能体创建",
  "在 GitHub Issues/PR 中使用 {highlight}/opennovel{/highlight} 触发 AI 操作",
  "运行 {highlight}opennovel github install{/highlight} 设置 GitHub 工作流",
  "在 Issues 中评论 {highlight}/opennovel fix this{/highlight} 以自动创建 PR",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.opennovel/themes/{/highlight} directory",
  ...
  "运行 {highlight}opennovel debug config{/highlight} 排查配置问题",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/marsqiu007/opennovel{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with OpenNovel for curated, tested models",
