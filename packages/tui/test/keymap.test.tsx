...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOpenNovelKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opennovel mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOpenNovelModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOpenNovelModeStack(keymap).push("autocomplete")
