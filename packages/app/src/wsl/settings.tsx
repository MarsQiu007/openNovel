...
import { wslOpenNovelAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.opennovelChecks[item.config.distro]
          const opennovelAction = () => wslOpenNovelAction(check())
          const busy = () => wsl.data?.job?.kind === "install-opennovel" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={opennovelAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOpenNovel(item.config.distro))}
                    >
