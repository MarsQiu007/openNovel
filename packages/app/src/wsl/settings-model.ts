...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-opennovel" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpenNovelAction(check?: WslOpenNovelCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install OpenNovel"
  if (check.matchesDesktop === false) return "Update OpenNovel"
}
...
export function addServerViewModel(input: {
  ...
  const opennovelCheck = selectedDistro ? (state?.opennovelChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.opennovelChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateOpenNovel" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.opennovelMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installOpenNovel" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  opennovelCheck: WslOpenNovelCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingOpenNovel =
    probingSelected ||
    (ready &&
      (!input.opennovelCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingOpenNovel =
    input.state?.job?.kind === "install-opennovel" && input.state.job.distro === input.selectedDistro
  if (!ready || probingOpenNovel) {
    return {
      variant: "contrast",
  ...
  if (!addServerOpenNovelReady(input.opennovelCheck)) {
    const update = !!input.opennovelCheck?.resolvedPath && input.opennovelCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingOpenNovel
        ? { key: "wsl.onboarding.updatingOpenNovel" }
        : update
          ? { key: "wsl.onboarding.updateOpenNovel" }
          : { key: "wsl.onboarding.installOpenNovel" },
      disabled: !!input.state?.job || input.adding,
      action: "install-opennovel",
      loading: installingOpenNovel,
...
function addServerOpenNovelReady(check: WslOpenNovelCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.opennovelChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.opennovelChecks[item.name]) return [`opennovel:${item.name}`]
    return []
  })
