...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when OpenNovel needs attention", () => {
    expect(wslOpenNovelAction(undefined)).toBeUndefined()
    expect(
      wslOpenNovelAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install OpenNovel")
    expect(
      wslOpenNovelAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/opennovel",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update OpenNovel")
    expect(
      wslOpenNovelAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/opennovel",
  ...
  test("does not report ready when OpenNovel is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        opennovelChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.opennovel/bin/opennovel",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installOpenNovel" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-opennovel")
  })
