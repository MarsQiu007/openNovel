...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("opennovel://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("opennovel://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("opennovel://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("opennovel://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("opennovel://open-project")).toBeUndefined()
    expect(parseDeepLink("opennovel://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "opennovel://open-project?directory=/a",
      "opennovel://other?directory=/b",
      "opennovel://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("opennovel://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("opennovel://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("opennovel://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("opennovel://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "opennovel://new-session?directory=/a",
      "opennovel://open-project?directory=/b",
      "opennovel://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENNOVEL__: {
        deepLinks: ["opennovel://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENNOVEL__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["opennovel://open-project?directory=/a"])
