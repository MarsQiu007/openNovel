 ...
 *
 * 单一执行入口按目标类型路由 prompt 构建：chapter 走章节批量修订流程，
 * world_entry 走设定受控修改流程；两条 prompt 模板与状态机原样保留。
 */
...
export function formatSettingExecutionPrompt(input: {
  ...
  const sections = [
    ...
    "\n## 执行规则\n"
      ...
      + "7. 成功或失败都必须调用 report_annotation_execution 回填 execution_round_id、状态和结果摘要。",
  ]
  return sections.join("\n")
}
...
export async function executeAnnotationExecution(
  input: {
    ...
    readonly targetTitle?: string | null | undefined
    readonly paragraphs: readonly string[]
    readonly annotations: readonly AnnotationExecutionInput[]
  },
  deps: {
  ...
  try {
    const prompt =
      input.targetType === "world_entry"
        ? formatSettingExecutionPrompt({
            roundID: round.id,
            entryID: input.targetID,
            entryTitle: input.targetTitle,
        ...
        : formatExecutionPrompt({
            ...
            chapterTitle: input.targetTitle,
            paragraphs: input.paragraphs,
            annotations: input.annotations,
          })
