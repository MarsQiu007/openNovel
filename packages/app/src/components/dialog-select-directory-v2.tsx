...
import { Dialog, DialogBody, DialogFooter, DialogHeader, DialogTitle } from "@opennovel-ai/ui/v2/dialog-v2"
...
import { DividerV2 } from "@opennovel-ai/ui/v2/divider-v2"
...
export function DialogSelectDirectoryV2(props: DialogSelectDirectoryV2Props) {
  ...
  // 打开系统目录选择器（桌面端原生对话框 / web 端复用一个目录选择对话框），
  // 选中后把路径作为根目录填入本对话框并加载目录树。
  const browse = async () => {
    if (platform.platform === "desktop") {
      const path = await platform.openDirectoryPickerDialog({ title: props.title, multiple: false })
      const picked = Array.isArray(path) ? path[0] : path
      if (picked) void navigate(picked)
      return
    }
    // web 端：弹出一个目录选择对话框，选中后回填
    let chosen: string | null = null
    const onSelect = (result: string | string[] | null) => {
      chosen = Array.isArray(result) ? result[0] : result
    }
    const cancel = () => {
      if (chosen != null) void navigate(chosen)
    }
    dialog.show(
      () => <DialogSelectDirectoryV2 title="" server={props.server} onSelect={onSelect} start={root() || start()} />,
      cancel,
    )
  }
        ...
        <div class="directory-picker-v2-path" ref={pathArea}>
          ...
          <div class="directory-picker-v2-actions">
            <ButtonV2
              size="small"
              variant="ghost"
              title={language.t("dialog.directory.browse")}
              onClick={() => void browse()}
            >
              ...
            </ButtonV2>
