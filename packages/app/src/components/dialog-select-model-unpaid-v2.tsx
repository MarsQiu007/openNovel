...
import { useTheme } from "@opennovel-ai/ui/theme"
...
const featuredProviders = ["opennovel", "opennovel-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "opennovel" && (!item.cost || item.cost.input === 0)
