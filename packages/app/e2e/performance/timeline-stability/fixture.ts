...
import { Event } from "@opennovel-ai/schema/event"
import { SessionStatusEvent } from "@opennovel-ai/schema/session-status-event"
import { SessionV1 } from "@opennovel-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/OpenNovel/TimelineStability"
...
export const model = { providerID: "opennovel", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("opennovel.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["opennovel"],
    default: { providerID: "opennovel", modelID: "claude-opus-4-6" },
  }
}
