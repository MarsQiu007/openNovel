...
import { createChapterAnnotationEndpoint, createWorldEntryAnnotation } from "../src/handlers/novel"
...
describe("章节批注锚点校验", () => {
  test("接受合法的单段批注并持久化", async () => {
    const ann = await Effect.runPromise(
      createChapterAnnotationEndpoint(chapterId, novelId, {
        anchorType: "range",
        paragraphIndex: 0,
        startOffset: 0,
        endOffset: 4,
  ...
  test("整章批注无需锚点字段", async () => {
    const ann = await Effect.runPromise(
      createChapterAnnotationEndpoint(chapterId, novelId, { anchorType: "chapter", comment: "整体评价" }, tempDir),
    )
  ...
  test("章节不存在时返回 not found", async () => {
    await expectAnchorError(
      createChapterAnnotationEndpoint("no-such-chapter", novelId, {
...
describe("设定批注锚点校验（跨段扩展）", () => {
  test("接受合法的跨段批注并持久化 endParagraphIndex", async () => {
    const ann = await Effect.runPromise(
      createWorldEntryAnnotation(novelId, entryId, {
