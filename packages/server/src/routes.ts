...
import { LocationServiceMap } from "@opennovel-ai/core/location-service-map"
import { SessionExecutionLocal } from "@opennovel-ai/core/session/execution/local"
...
export function createRoutes(password?: string) {
  return makeRoutes(
    password
      ? ServerAuth.Config.configLayer({ username: "opennovel", password: Option.some(password) })
      : ServerAuth.Config.layer,
  )
}

export function createEmbeddedRoutes() {
  return makeRoutes(ServerAuth.Config.configLayer({ username: "opennovel", password: Option.none() }))
}
