...
import { createOpenNovelClient } from "@opennovel-ai/sdk/v2/client"
import { ServerAuth } from "@opennovel-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createOpenNovelClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@opennovel/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createOpenNovelClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createOpenNovelClient({ baseUrl: connection.url, headers: connection.headers })
    })
