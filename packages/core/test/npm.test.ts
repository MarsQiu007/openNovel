...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@opennovel/acme")).toBe("@opennovel/acme")
    expect(Npm.sanitize("@opennovel/acme@1.0.0")).toBe("@opennovel/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/opennovel/acme.git"
    const expected = win ? "acme@git+https_//github.com/opennovel/acme.git" : spec
