...
const opennovelClient = process.env.OPENNOVEL_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opennovelClient === undefined) delete process.env.OPENNOVEL_CLIENT
  else process.env.OPENNOVEL_CLIENT = opennovelClient
})
...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["opennovel.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.OPENNOVEL_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "opennovel.client=web,service.instance.id=override,service.namespace=anomalyco"

    expect(resource().attributes).toMatchObject({
      "opennovel.client": "cli",
      "service.namespace": "anomalyco",
    })
    expect(resource().attributes["service.instance.id"]).not.toBe("override")
    expect(resource().attributes["opennovel.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opennovel-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "opennovel.log")
