...
import { PluginHost } from "@opennovel-ai/core/plugin/host"
import { OpenNovelPlugin } from "@opennovel-ai/core/plugin/provider/opennovel"
...
const addPlugin = Effect.fn(function* () {
  ...
  yield* OpenNovelPlugin.effect(host).pipe(
...
describe("OpenNovelPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("opennovel")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "OpenNovel Console account",
        },
  ...
  it.live("loads providers and models from the connected OpenNovel server", () =>
      ...
      ({ authorization, release, server }) =>
        Effect.gen(function* () {
          ...
          yield* credentials.create({
            integrationID: Integration.ID.make("opennovel"),
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("opennovel"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "opennovel",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.opennovel),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opennovel)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.opennovel, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opennovel)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.opennovel, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opennovel)).request.body.apiKey).toBe("public")
        expect(
          required(yield* catalog.model.get(ProviderV2.ID.opennovel, ModelV2.ID.make("output-only"))).enabled,
        ).toBe(true)
      }),
    ),
  )

  it.effect("uses OPENNOVEL_API_KEY as credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opennovel)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.opennovel, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined, CUSTOM_OPENNOVEL_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opennovel"),
            method: { type: "env", names: ["CUSTOM_OPENNOVEL_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opennovel)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.opennovel, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-opennovel providers and models", () =>
    withEnv({ OPENNOVEL_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the opennovel small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.opennovel
