...
import { ModelsDevPlugin } from "@opennovel-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.OPENNOVEL_MODELS_PATH,
          disabled: Flag.OPENNOVEL_DISABLE_MODELS_FETCH,
        }
        Flag.OPENNOVEL_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.OPENNOVEL_MODELS_PATH = previous.path
          Flag.OPENNOVEL_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
