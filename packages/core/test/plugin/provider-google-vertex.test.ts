...
import { GoogleVertexPlugin } from "@opennovel-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.opennovel, (provider) => {
          provider.api = {
            ...
            url: "https://opennovel.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.opennovel))
