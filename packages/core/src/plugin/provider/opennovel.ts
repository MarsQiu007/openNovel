...
import type { IntegrationOAuthMethodRegistration } from "@opennovel-ai/plugin/v2/effect/integration"
import { define } from "@opennovel-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@opennovel-ai/sdk/v2/types"
...
const defaultServer = "https://console.opennovel.ai"
const clientID = "opennovel-cli"
...
export const OpenNovelPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "opennovel",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpenNovelPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("opennovel")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load OpenNovel provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("opennovel", (integration) => {
        integration.name = "OpenNovel"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "opennovel", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("opennovel")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("opennovel")
      ...
      const item = catalog.provider.get(ProviderV2.ID.opennovel)
      if (!item) return
      const hasKey = Boolean(process.env.OPENNOVEL_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("opennovel")),
