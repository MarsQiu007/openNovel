...
import { ToolFailure } from "@opennovel-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@opennovel/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OPENNOVEL_WEBSEARCH_PROVIDER === "exa" || process.env.OPENNOVEL_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OPENNOVEL_WEBSEARCH_PROVIDER
        : undefined,
    enableExa:
      truthy("OPENNOVEL_EXPERIMENTAL") || truthy("OPENNOVEL_ENABLE_EXA") || truthy("OPENNOVEL_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OPENNOVEL_ENABLE_PARALLEL") || truthy("OPENNOVEL_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `opennovel/${InstallationVersion}`,
