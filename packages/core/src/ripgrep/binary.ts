...
export namespace RipgrepBinary {
  ...
  export class Service extends Context.Service<Service, Interface>()("@opennovel/RipgrepBinary") {}
    ...
    Effect.gen(function* () {
      ...
      const extract = Effect.fnUntraced(function* (
        ...
        if (config.extension === "zip") {
          // bsdtar ships with Windows 10+ and extracts zips far faster than
          // Expand-Archive, which stalls under Defender real-time scanning in CI.
          const systemTar = path.join(process.env.SystemRoot ?? "C:\\Windows", "System32", "tar.exe")
          const tar = (yield* fs.isFile(systemTar).pipe(Effect.orElseSucceed(() => false)))
            ? systemTar
            : yield* Effect.sync(() => which("tar.exe") ?? which("tar"))
          const extracted = tar ? yield* run(tar, ["-xf", archive, "-C", dir]) : undefined
          if (!extracted || extracted.code !== 0) {
            if (extracted)
              yield* Effect.logWarning("ripgrep tar extraction failed; falling back to Expand-Archive", {
                code: extracted.code,
