...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("opennovel-find")
  api.delete("opennovel-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("opennovel-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("opennovel-find", new Highlight(...rest))
    return true
  }
