---
title: Konfiguration
description: Ved at bruge OpenNovel JSON konfig.
---

Du kan konfigurere OpenNovel ved hjælp af en JSON-konfigurationsfil.

---

## Format

OpenNovel understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/opennovel`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/opennovel/opennovel.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENNOVEL_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`opennovel.json` i projekt) - projektspecifikke indstillinger
5. **`.opennovel` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OPENNOVEL_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.opennovel` og `~/.config/opennovel` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/opennovel`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenNovel-konfiguration i `~/.config/opennovel/opennovel.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `opennovel.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når OpenNovel starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OPENNOVEL_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OPENNOVEL_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.opennovel` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.opennovel` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`opennovel.ai/config.json`**](https://opennovel.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `opennovel serve` og `opennovel web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din OpenNovel-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `opennovel.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din OpenNovel-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger OpenNovel at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din OpenNovel-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opennovel/agents/` eller `.opennovel/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OpenNovel falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`opennovel run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opennovel/commands/` eller `.opennovel/commands/`. [Learn more here](/docs/commands).
...
OpenNovel vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard opennovel **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide OpenNovel med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.opennovel/plugins/` eller `~/.config/opennovel/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse OpenNovel til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
