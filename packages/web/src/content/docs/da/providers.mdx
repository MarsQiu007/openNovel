---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenNovel.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenNovel bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenNovel-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/opennovel/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenNovel-konfigurationen.
...
OpenNovel Zen er en liste over modeller leveret af OpenNovel-teamet, som er blevet testet og verificeret til at fungere godt med OpenNovel. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenNovel Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opennovel og gå til [opennovel.ai/auth](https://opennovel.ai/auth).
...
Det fungerer som alle andre udbydere i OpenNovel og er helt valgfrit at bruge.

---

## OpenNovel Go

OpenNovel Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenNovel-teamet, som er testet og verificeret til at fungere godt med OpenNovel.

1. Kør kommandoen `/connect` i TUI, vælg `OpenNovel Go`, og gå til [opennovel.ai/auth](https://opennovel.ai/zen).
...
Sådan bruger du Amazon Bedrock med OpenNovel:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører opennovel:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `opennovel.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenNovel understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere opennovel til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at opennovel skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem opennovel-konfigurationen.
...
gemme tokenet i opennovel auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenNovel bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenNovel
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`opennovel.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `opennovel.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenNovel:
...
Sådan bruger du Google Vertex AI med OpenNovel:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører opennovel.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem opennovel, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) plugin for automatisk at logge hver OpenNovel-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenNovel-samtale opført som en separat session.
...
Du kan konfigurere opennovel til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere opennovel til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere opennovel til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenNovel. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opennovel) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenNovel:
...
3. Kopier API-nøglen til brug i OpenNovel.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenNovel, skal du hente modelinformationen lokalt:
...
OpenNovel Zen er en liste over testede og verificerede modeller leveret af OpenNovel-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenNovel Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenNovel Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenNovel API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem opennovel-konfigurationen.
...
5. Du kan også tilpasse dem gennem opennovel-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenNovel:
...
Du kan også tilpasse modeller gennem opennovel-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med opennovel. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `opennovel.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenNovel forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `opennovel auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek opennovel-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i opennovel-konfigurationen.
