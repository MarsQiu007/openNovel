...
OpenNovel Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i OpenNovel. Du abonnerer på OpenNovel Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med OpenNovel.
:::
...
OpenNovel Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

OpenNovel Go fungerer som enhver anden udbyder i OpenNovel.

1. Du logger ind på **<a href={console}>OpenNovel Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `OpenNovel Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på OpenNovel Go.
:::
...
OpenNovel Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din OpenNovel config
bruger formatet `opennovel-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `opennovel-go/kimi-k3` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med OpenNovel.
