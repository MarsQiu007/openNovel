---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenNovel.
---

Plugins giver dig mulighed for at udvide OpenNovel ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenNovels standardadfærd.
...
- `.opennovel/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/opennovel/node_modules/`.
...
1. Global konfiguration (`~/.config/opennovel/opennovel.json`)
2. Projektkonfiguration (`opennovel.json`)
3. Global plugin-mappe (`~/.config/opennovel/plugins/`)
4. Projekt plugin bibliotek (`.opennovel/plugins/`)
...
OpenNovel kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En opennovel SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide opennovel.
...
:::note
Hvis du bruger OpenNovel desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå opennovel i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til opennovel:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som opennovel kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for opennovel sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://opennovel.ai/docs/sdk) for detaljer.
