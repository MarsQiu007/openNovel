---
title: Server
description: Komunicirajte s opennovel serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `opennovel serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opennovel klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `opennovel.local` |
...
Postavite `OPENNOVEL_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opennovel`, ili postavite `OPENNOVEL_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `opennovel serve` i za `opennovel web`.
...
Kada pokrenete `opennovel`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite opennovel server za programsku interakciju sa opennovel-om.
:::

Ova arhitektura omogucava opennovel podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `opennovel serve` da startate standalone server. Ako je opennovel TUI vec pokrenut, `opennovel serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenNovel [IDE](/docs/ide) pluginovi.
...
opennovel server izlaže sljedece API-je.
