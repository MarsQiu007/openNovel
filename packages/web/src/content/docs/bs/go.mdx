...
OpenNovel Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u OpenNovel-u. Pretplatite se na OpenNovel Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili OpenNovel.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa OpenNovel-om.
:::
...
OpenNovel Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

OpenNovel Go radi kao bilo koji drugi provajder u OpenNovel-u.

1. Prijavite se na **<a href={console}>OpenNovel Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `OpenNovel Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na OpenNovel Go.
:::
...
OpenNovel Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj OpenNovel konfiguraciji
koristi format `opennovel-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`opennovel-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo OpenNovel Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz OpenNovel koristite i bilo kojeg drugog provajdera.
