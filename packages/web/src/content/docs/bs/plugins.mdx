---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenNovel.
---

Dodaci vam omogućavaju da proširite OpenNovel spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenNovel.
...
- `.opennovel/plugins/` - Dodaci na nivou projekta
- `~/.config/opennovel/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/opennovel/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/opennovel/opennovel.json`)
2. Konfiguracija projekta (`opennovel.json`)
3. Globalni direktorij dodataka (`~/.config/opennovel/plugins/`)
4. Direktorij dodataka projekta (`.opennovel/plugins/`)
...
OpenNovel pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: OpenNovel SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenNovel.
...
Spriječite opennovel da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u opennovel:
...
Pomoćnik `tool` kreira prilagođeni alat koji opennovel može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za opennovel zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://opennovel.ai/docs/sdk) za detalje.
