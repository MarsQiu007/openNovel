...
Da biste otklonili probleme s OpenNovel, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opennovel\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `opennovel --log-level DEBUG`.

---

## Pohrana

OpenNovel pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opennovel/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\opennovel`
...
OpenNovel Desktop pokreće lokalni OpenNovel server (`opennovel-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenNovel` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opennovel/opennovel.jsonc` (ili `~/.config/opennovel/opennovel.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opennovel/opennovel.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opennovel\opennovel.jsonc`
...
OpenNovel također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opennovel/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opennovel\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenNovel mogao ponovo izgraditi.

1. Potpuno zatvorite OpenNovel Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/opennovel`
- **Linux**: obrišite `~/.cache/opennovel` (ili pokrenite `rm -rf ~/.cache/opennovel`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\opennovel`

3. Ponovo pokrenite OpenNovel Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenNovel Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `opennovel.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENNOVEL_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENNOVEL_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenNovel Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenNovel karakteristikama.

---

### Obavještenja se ne prikazuju

OpenNovel Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenNovel u postavkama vašeg OS-a, i
...
1. Zatvorite OpenNovel Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenNovel Desktop aplikacije):

- `opennovel.settings.dat` (URL zadanog servera za desktop)
- `opennovel.global.dat` i `opennovel.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenNovel:
...
Da saznate kojim modelima imate pristup, pokrenite `opennovel models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\opennovel`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenNovel dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\opennovel`

2. Ponovo pokrenite OpenNovel da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenNovel da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenNovel će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
