...
U OpenNovel mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenNovel koristi nasu `opennovel` temu.
...
OpenNovel dolazi sa vise ugradenih tema.
...
- Zele da OpenNovel odgovara izgledu njihovog terminala
...
OpenNovel podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/opennovel/themes/*.json` ili `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.opennovel/themes/*.json`
4. **Trenutni radni direktorij** - `./.opennovel/themes/*.json`
