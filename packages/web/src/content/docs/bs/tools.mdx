...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenNovel dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenNovel.
...
:::note
Ovaj alat je dostupan samo kada je `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenNovel provajdera ili kada je varijabla `OPENNOVEL_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenNovel:
