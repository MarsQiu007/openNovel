...
Možete dodati vanjske alate u OpenNovel koristeći _Model Context Protocol_, ili MCP. OpenNovel podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenNovel Config](https://opennovel.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/opennovel`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenNovel automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenNovel će:
...
Ako server zahtijeva autentifikaciju, OpenNovel će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opennovel mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenNovel sigurno čuva tokene u `~/.local/share/opennovel/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenNovel, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenNovel konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenNovel sa vašim Sentry nalogom.
