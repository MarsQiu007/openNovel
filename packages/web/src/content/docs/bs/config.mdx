---
title: Konfiguracija
description: Korištenje OpenNovel JSON konfiguracije.
---

Možete konfigurirati OpenNovel koristeći JSON konfiguracijski fajl.

---

## Format

OpenNovel podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/opennovel`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/opennovel/opennovel.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENNOVEL_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`opennovel.json` u projektu) - postavke specifične za projekat
5. **`.opennovel` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENNOVEL_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.opennovel` i `~/.config/opennovel` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/opennovel` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenNovel konfiguraciju u `~/.config/opennovel/opennovel.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/opennovel/tui.json`.
...
Dodajte `opennovel.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OpenNovel pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENNOVEL_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENNOVEL_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opennovel` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.opennovel`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`opennovel.ai/config.json`**](https://opennovel.ai/config.json).

TUI konfiguracija koristi [**`opennovel.ai/tui.json`**](https://opennovel.ai/tui.json).
...
Koristite `OPENNOVEL_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `opennovel.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `opennovel serve` i `opennovel web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenNovel server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `opennovel.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenNovel konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenNovel pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opennovel/agents/` ili `.opennovel/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenNovel će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`opennovel run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opennovel/commands/` ili `.opennovel/commands/`. [Saznajte više ovdje](/docs/commands).
...
OpenNovel će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenNovel **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenNovel sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.opennovel/plugins/` ili `~/.config/opennovel/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OpenNovel da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
