---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenNovel.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenNovel koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenNovel konfiguraciji.
...
u `~/.local/share/opennovel/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenNovel
config.
...
OpenNovel Zen je lista modela koje je obezbedio OpenNovel tim koji su bili
testirano i potvrđeno da dobro radi sa OpenNovel. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenNovel Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite opennovel i idite na [opennovel.ai/auth](https://opennovel.ai/auth).
...
Radi kao i svaki drugi provajder u OpenNovel i potpuno je opcionalan za korištenje.

---

## OpenNovel Go

OpenNovel Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenNovel tim i koji su testirani i verificirani da dobro rade s OpenNovel-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenNovel Go` i idite na [opennovel.ai/auth](https://opennovel.ai/zen).
...
Da biste koristili Amazon Bedrock s OpenNovel:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete opennovel:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `opennovel.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenNovel.
:::
...
Možete konfigurirati opennovel za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi opennovel ispravno radio.
   :::
...
Također možete dodati modele kroz svoju opennovel konfiguraciju.
...
da pohrani token u opennovel auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenNovel koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenNovel
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`opennovel.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth).

##### Konfiguracija

Prilagodite putem `opennovel.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s opennovel:
...
Za korištenje Google Vertex AI s OpenNovel:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće opennovel.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem opennovela, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) da automatski prijavite svaki OpenNovel razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenNovel razgovor naveden kao zasebna sesija.
...
Možete konfigurirati opennovel za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati opennovel za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati opennovel za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenNovel. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opennovel) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenNovel:
...
3. Kopirajte API ključ za korištenje u OpenNovel.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenNovel, morate lokalno povući informacije o modelu:
...
OpenNovel Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenNovel tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenNovel Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenNovel Zen**.

```txt
   /connect
```

3. Unesite svoj OpenNovel API ključ.
...
Također možete dodati dodatne modele putem vaše opennovel konfiguracije.
...
5. Također ih možete prilagoditi putem vaše opennovel konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa OpenNovelom:
...
Također možete prilagoditi modele kroz svoju opennovel konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s opennovel-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `opennovel.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenNovel da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `opennovel auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenNovel konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj opennovel konfiguraciji.
