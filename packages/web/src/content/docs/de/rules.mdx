---
title: Regeln
description: Lege eigene Anweisungen fuer opennovel fest.
---

Du kannst opennovel ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in opennovel den Befehl `/init` aus.
...
So kann opennovel sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht opennovel sie zu erweitern.
...
opennovel kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/opennovel/AGENTS.md` speichern.
Diese gelten in allen opennovel-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenNovel dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/opennovel/AGENTS.md` existiert)
...
Beim Start sucht opennovel in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/opennovel/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `opennovel.json` oder in `~/.config/opennovel/opennovel.json` angeben.
...
opennovel parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit opennovel.json

Empfohlen ist das `instructions`-Feld in der `opennovel.json`:
...
Du kannst opennovel explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `opennovel.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
