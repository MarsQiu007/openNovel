...
Mit OpenNovel kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenNovel das `opennovel`-Theme.
...
OpenNovel liefert mehrere eingebaute Themes mit.
...
- OpenNovel optisch ans Terminal anpassen wollen
...
OpenNovel unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/opennovel/themes/*.json` oder `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.opennovel/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.opennovel/themes/*.json`
