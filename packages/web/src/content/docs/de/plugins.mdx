---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenNovel zu erweitern.
---

Mit Plugins können Sie OpenNovel erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenNovel zu ändern.
...
- `.opennovel/plugins/` – Plugins auf Projektebene
- `~/.config/opennovel/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/opennovel/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/opennovel/opennovel.json`)
2. Projektkonfiguration (`opennovel.json`)
3. Globales Plugin-Verzeichnis (`~/.config/opennovel/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.opennovel/plugins/`)
...
OpenNovel führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenNovel-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenNovel erweitern können.
...
:::note
Wenn Sie die OpenNovel-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenNovel `.env`-Dateien liest:
...
Plugins können OpenNovel auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenNovel aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenNovel zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://opennovel.ai/docs/sdk).
