...
Es gibt zwei Arten von Agenten in OpenNovel; Primäragenten und Subagenten.
...
OpenNovel verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `opennovel.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet OpenNovel modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenNovel-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenNovel-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenNovel Zen](/docs/zen) verwenden, würden Sie `opennovel/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `opennovel models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/opennovel).
:::
