---
title: Server
description: Nutze den opennovel-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `opennovel serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein opennovel-Client nutzen kann.
...
Setze `OPENNOVEL_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `opennovel`, kann aber mit `OPENNOVEL_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `opennovel serve` und `opennovel web`.
...
Wenn du `opennovel` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opennovel-Server, um opennovel programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `opennovel serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `opennovel serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenNovel-[IDE](/docs/ide)-Plugins.
...
Der opennovel-Server stellt folgende APIs bereit.
