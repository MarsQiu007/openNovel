...
Agent Skills erlauben OpenNovel, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenNovel sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenNovel vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.opennovel/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/opennovel/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.opennovel/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenNovel listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `opennovel.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `opennovel.json`):
