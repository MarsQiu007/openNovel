...
Wenn OpenNovel Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\opennovel\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `opennovel --log-level DEBUG`.

---

## Speicher

opennovel speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opennovel/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\opennovel`
...
OpenNovel Desktop startet im Hintergrund einen lokalen OpenNovel-Server (`opennovel-cli`-Sidecar).
...
- Nur macOS: `OpenNovel`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opennovel/opennovel.jsonc` (oder `~/.config/opennovel/opennovel.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opennovel/opennovel.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opennovel\opennovel.jsonc`
...
OpenNovel kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opennovel/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opennovel\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenNovel ihn neu aufbauen kann.

1. Beende OpenNovel Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/opennovel`
- **Linux**: loesche `~/.cache/opennovel` (oder fuehre aus: `rm -rf ~/.cache/opennovel`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\opennovel`

3. Starte OpenNovel Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenNovel Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `opennovel.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENNOVEL_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENNOVEL_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt OpenNovel Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenNovel-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenNovel Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenNovel in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenNovel Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenNovel Desktop):

- `opennovel.settings.dat` (desktop default server URL)
- `opennovel.global.dat` und `opennovel.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenNovel hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `opennovel models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\opennovel`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. opennovel installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\opennovel`

2. Starte opennovel neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt opennovel, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
opennovel erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
