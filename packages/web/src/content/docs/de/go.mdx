...
OpenNovel Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in OpenNovel. Du abonnierst OpenNovel Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
OpenNovel zu verwenden.
...
OpenNovel Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

OpenNovel Go funktioniert wie jeder andere Provider in OpenNovel.

1. Du meldest dich bei **<a href={console}>OpenNovel Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenNovel Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenNovel Go abonnieren.
:::
...
OpenNovel Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner OpenNovel Config verwendet das Format `opennovel-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `opennovel-go/kimi-k3` in deiner Config verwenden.
...
Wir haben OpenNovel Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OpenNovel zu nutzen.
