---
title: Eklentiler
description: OpenNovel'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenNovel'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenNovel'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.opennovel/plugins/` - Proje düzeyinde eklentiler
- `~/.config/opennovel/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/opennovel/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/opennovel/opennovel.json`)
2. Project config (`opennovel.json`)
3. Global eklenti dizini (`~/.config/opennovel/plugins/`)
4. Proje eklenti dizini (`.opennovel/plugins/`)
...
OpenNovel bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenNovel SDK istemcisi.
...
OpenNovel'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenNovel masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenNovel'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenNovel'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenNovel'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenNovel'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://opennovel.ai/docs/sdk) bakın.
