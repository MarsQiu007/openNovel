...
description: OpenNovel, LSP sunucularınızla bütünleşir.
---

OpenNovel, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

OpenNovel, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OPENNOVEL_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve opennovel bir dosyayı açtığında:
...
LSP sunucularını, opennovel yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
