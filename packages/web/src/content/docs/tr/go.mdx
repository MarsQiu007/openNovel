...
OpenNovel Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OpenNovel'daki diğer sağlayıcılar gibi çalışır. OpenNovel Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenNovel'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenNovel Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenNovel Go, OpenNovel'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenNovel Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenNovel Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenNovel Go'ya abone olabilir.
:::
...
OpenNovel Go aşağıdaki limitleri içerir:
...
OpenNovel yapılandırmanızdaki [model id](/docs/config/#models) formatı `opennovel-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opennovel-go/kimi-k3` kullanmalısınız.
...
OpenNovel Go'yu şu amaçlarla oluşturduk:
...
4. OpenNovel ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
