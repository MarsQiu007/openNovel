...
opennovel'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
opennovel, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
OpenNovel, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

OpenNovel iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `opennovel.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse opennovel modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global opennovel yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
opennovel yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenNovel Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opennovel/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `opennovel models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/opennovel).
:::
