---
title: Sunucu
description: opennovel sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`opennovel serve` komutu, opennovel istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `opennovel.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENNOVEL_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opennovel` değeridir; değiştirmek isterseniz `OPENNOVEL_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `opennovel serve` hem de `opennovel web` için geçerlidir.
...
`opennovel` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
opennovel ile programatik etkileşim için opennovel sunucusunu kullanın.
:::

Bu mimari, opennovel'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`opennovel serve` ile bağımsız bir sunucu başlatabilirsiniz. opennovel TUI açıksa, `opennovel serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum opennovel [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
opennovel sunucusu aşağıdaki API'leri sunar.
