---
title: Sağlayıcılar
description: opennovel'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

opennovel, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı opennovel yapılandırmanızda yapılandırın.
...
`~/.local/share/opennovel/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları opennovel'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenNovel Zen, opennovel ekibi tarafından sağlanan modellerin bir listesidir.
opennovel ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenNovel Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opennovel'u seçin ve [opennovel.ai/auth](https://opennovel.ai/auth)'ye gidin.
...
opennovel'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenNovel Go

OpenNovel Go, opennovel ile iyi çalıştığı test edilmiş ve doğrulanmış, opennovel ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenNovel Go`'yu seçin ve [opennovel.ai/auth](https://opennovel.ai/zen) adresine gidin.
...
Amazon Bedrock'u opennovel ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   opennovel'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `opennovel.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi opennovel'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
opennovel'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   opennovel'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   OpenNovel yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
opennovel, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. opennovel'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`opennovel.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth) ana sayfasında.

##### Yapılandırma

`opennovel.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi opennovel'la kullanmak için:
...
Google Vertex AI'yi opennovel ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları opennovel çalıştırılırken ayarlayın.
...
Helicone'un opennovel aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her opennovel görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her opennovel görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
opennovel'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla opennovel'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla opennovel'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini opennovel için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opennovel)'a bakın.
:::
...
Ollama Cloud'u opennovel ile kullanmak için:
...
3. opennovel'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: opennovel'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenNovel Zen, opennovel ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenNovel Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenNovel Zen**'i arayın.

   ```txt
   /connect
   ```

3. opennovel API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   opennovel yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca opennovel yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi OpenNovel ile kullanmak için:
...
Modelleri opennovel yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı opennovel ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `opennovel.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları opennovel'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `opennovel auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için opennovel yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, opennovel yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
