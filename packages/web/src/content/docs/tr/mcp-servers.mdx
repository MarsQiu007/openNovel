...
_Model Bağlam Protokolü_ veya MCP kullanarak opennovel'a harici araçlar ekleyebilirsiniz. opennovel hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [opennovel Config](https://opennovel.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/opennovel` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
opennovel, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde opennovel şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa opennovel, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `opennovel mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından opennovel, belirteçleri `~/.local/share/opennovel/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra opennovel'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi opennovel yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve opennovel'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
