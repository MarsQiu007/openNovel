...
opennovel ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak opennovel kendi `opennovel` temasını kullanır.
...
opennovel birden fazla yerleşik temayla gelir.
...
- opennovel'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
opennovel, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/opennovel/themes/*.json` veya `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.opennovel/themes/*.json`
4. **Geçerli çalışma dizini** - `./.opennovel/themes/*.json`
