...
Dzięki opennovel możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie opennovel używa naszego własnego motywu `opennovel`.
...
opennovel ma kilka wbudowanych motywów.
...
- Chcą, aby OpenNovel pasował do wyglądu ich terminala
...
opennovel obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/opennovel/themes/*.json` lub `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.opennovel/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.opennovel/themes/*.json`
