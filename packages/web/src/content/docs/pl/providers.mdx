---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w opennovel.
---

import config from "../../../../config.mjs"
export const console = config.console

opennovel używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji opennovel.
...
w `~/.local/share/opennovel/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim opennovel
config.
...
OpenNovel Zen to lista modeli dostarczonych przez zespół opennovel, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opennovel. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenNovel Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenNovel Zen` i przejdź do [opennovel.ai/auth](https://opennovel.ai/zen).
...
Działa jak każdy inny dostawca w opennovel i jest całkowicie opcjonalny w użyciu.

---

## OpenNovel Go

OpenNovel Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opennovel, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opennovel.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenNovel Go` i przejdź do [opennovel.ai/auth](https://opennovel.ai/zen).
...
Aby używać Amazon Bedrock z opennovel:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania opennovel:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `opennovel.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w opennovel nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować opennovel do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod opennovel działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji opennovel.
...
opennovel używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować opennovel
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`opennovel.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem opennovel:
...
Aby używać Google Vertex AI z opennovel:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania opennovel.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą opennovel, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session), aby automatycznie rejestrować każdą konwersację opennovel jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja opennovel będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować opennovel tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować opennovel tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować opennovel tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem opennovel. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opennovel).
:::
...
Aby korzystać z Ollama Cloud z opennovel:
...
3. Skopiuj klucz API do użycia w opennovel.
...
6. **Ważne**: Przed użyciem modeli chmurowych w opennovel musisz pobrać informacje o modelu lokalnie:
...
OpenNovel Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opennovel. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenNovel Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenNovel Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API opennovel.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji opennovel.
...
5. Możesz je także dostosować za pomocą konfiguracji opennovel. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z opennovel:
...
Możesz także dostosować modele za pomocą konfiguracji opennovel. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z opennovel. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `opennovel.json` w katalogu projektu:
...
Pola `limit` pozwalają opennovel zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `opennovel auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację opennovel i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji opennovel.
