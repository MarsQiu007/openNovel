...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenNovel posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenNovel.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenNovel lub gdy zmienna środowiskowa `OPENNOVEL_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenNovel:
