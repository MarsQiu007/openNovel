...
Aby debugować problemy z opennovel, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opennovel\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `opennovel --log-level DEBUG`.

---

## Przechowywanie danych

opennovel przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opennovel/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\opennovel`
...
opennovel Desktop uruchamia w tle lokalny serwer opennovel (sidecar `opennovel-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenNovel` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opennovel/opennovel.jsonc` (lub `~/.config/opennovel/opennovel.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opennovel/opennovel.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opennovel\opennovel.jsonc`
...
opennovel może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opennovel/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opennovel\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby opennovel mógł ją odbudować.

1. Całkowicie zamknij opennovel Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opennovel`
- **Linux**: usuń `~/.cache/opennovel` (lub uruchom `rm -rf ~/.cache/opennovel`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\opennovel`

3. Uruchom ponownie opennovel Desktop.

---

### Napraw problemy z połączeniem z serwerem

opennovel Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `opennovel.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENNOVEL_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENNOVEL_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows opennovel Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami opennovel.

---

### Brak powiadomień

opennovel Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla opennovel w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie opennovel Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji opennovel Desktop):

- `opennovel.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `opennovel.global.dat` i `opennovel.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z opennovel:
...
3. Upewnij się, że masz najnowszą wersję z `opennovel upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `opennovel models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\opennovel`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. opennovel dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\opennovel`

2. Uruchom ponownie kod opennovel, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to opennovel do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
opennovel wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
