---
title: Serwer
description: Interakcja z serwerem opennovel za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `opennovel serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opennovel.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `opennovel.local` |
...
Ustaw `OPENNOVEL_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opennovel` lub ustaw `OPENNOVEL_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `opennovel serve`, jak i `opennovel web`.
...
Kiedy uruchomisz `opennovel`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera opennovel do programowej interakcji z opennovel.
:::

Ta architektura umożliwia obsługę wielu klientów przez opennovel i programową interakcję z kodem otwartym.

Możesz uruchomić `opennovel serve`, aby uruchomić samodzielny serwer. Jeśli masz
opennovel TUI działa, `opennovel serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki opennovel [IDE](/docs/ide).
...
Serwer opennovel udostępnia następujące interfejsy API.
