---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć opennovel.
---

Wtyczki umożliwiające wyłączenie opennovel poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania opennovel.
...
- `.opennovel/plugins/` - Wtyczki na poziomie projektu
- `~/.config/opennovel/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/opennovel/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/opennovel/opennovel.json`)
2. Konfiguracja projektu (`opennovel.json`)
3. Globalny katalog wtyczek (`~/.config/opennovel/plugins/`)
4. Katalog wtyczek projektu (`.opennovel/plugins/`)
...
opennovel uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z opennovel do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do opennovel.
...
:::note
Jeśli korzystasz z aplikacji komputerowej opennovel, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj opennovel czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do opennovel:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod opennovel. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://opennovel.ai/docs/sdk).
