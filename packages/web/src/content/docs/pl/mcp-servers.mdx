...
Możesz dodać dodatkowe informacje do opennovel za pomocą _Model Context Protocol_, w skrócie MCP. opennovel obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [opennovel Config](https://opennovel.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/opennovel`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
opennovel automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opennovel:
...
Jeśli serwer wymaga uwierzytelnienia, opennovel poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opennovel mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji opennovel bezpieczne przechowa tokeny w `~/.local/share/opennovel/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w opennovel, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie opennovel, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć opennovel z kontem Sentry.
