...
Umiejętności agenta pozwalają opennovel odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
opennovel przeszukuje te lokalizacje:

- Project config: `.opennovel/skills/<name>/SKILL.md`
- Global config: `~/.config/opennovel/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu opennovel przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.opennovel/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/opennovel/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.opennovel/skills/git-release/SKILL.md` w ten sposób:
...
opennovel wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `opennovel.json`:
...
**Dla agentów wbudowanych** (w `opennovel.json`):
