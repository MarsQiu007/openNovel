---
title: Config
description: Using the OpenNovel JSON config.
---

You can configure OpenNovel using a JSON config file.

---

## Format

OpenNovel supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="opennovel.jsonc"
{
...
1. **Remote config** (from `.well-known/opennovel`) - organizational defaults
2. **Global config** (`~/.config/opennovel/opennovel.json`) - user preferences
3. **Custom config** (`OPENNOVEL_CONFIG` env var) - custom overrides
4. **Project config** (`opennovel.json` in project) - project-specific settings
5. **`.opennovel` directories** - agents, commands, plugins
6. **Inline config** (`OPENNOVEL_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/opennovel/` on macOS) - admin-controlled
...
:::note
The `.opennovel` and `~/.config/opennovel` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/opennovel` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/opennovel"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global OpenNovel config in `~/.config/opennovel/opennovel.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/opennovel/tui.json`.
...
Add `opennovel.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When OpenNovel starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `OPENNOVEL_CONFIG` environment variable.

```bash
export OPENNOVEL_CONFIG=/path/to/my/custom-config.json
opennovel run "Hello world"
```
...
Specify a custom config directory using the `OPENNOVEL_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.opennovel` directory, and should
follow the same structure.

```bash
export OPENNOVEL_CONFIG_DIR=/path/to/my/config-directory
opennovel run "Hello world"
```

The custom directory is loaded after the global config and `.opennovel` directories, so it **can override** their settings.
...
Drop an `opennovel.json` or `opennovel.jsonc` file in the system managed config directory:

| Platform | Path                                      |
| -------- | ----------------------------------------- |
| macOS    | `/Library/Application Support/opennovel/` |
| Linux    | `/etc/opennovel/`                         |
| Windows  | `%ProgramData%\opennovel`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenNovel reads managed preferences from the `ai.opennovel.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenNovel checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opennovel.managed.plist`
2. `/Library/Managed Preferences/ai.opennovel.managed.plist`

The plist keys map directly to `opennovel.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opennovel.managed` PayloadType. The OpenNovel config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.opennovel.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.opennovel.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.opennovel</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`opennovel.ai/config.json`**](https://opennovel.ai/config.json).

TUI config uses [**`opennovel.ai/tui.json`**](https://opennovel.ai/tui.json).
...
Use `OPENNOVEL_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `opennovel.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `opennovel serve` and `opennovel web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your OpenNovel server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `opennovel.local`. Useful for running multiple instances on the same network.
...
If not specified, OpenNovel will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your OpenNovel config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, OpenNovel tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny OpenNovel actions on configured resources. Currently, policies can control which providers OpenNovel may use.
...
OpenNovel normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, OpenNovel omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/opennovel/agents/` or `.opennovel/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenNovel will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`opennovel run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/opennovel/commands/` or `.opennovel/commands/`. [Learn more here](/docs/commands).
...
OpenNovel uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
OpenNovel will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, opennovel **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend OpenNovel with custom tools, hooks, and integrations.

Place plugin files in `.opennovel/plugins/` or `~/.config/opennovel/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="opennovel.json"
{
  "$schema": "https://opennovel.ai/config.json",
  "plugin": ["opennovel-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict OpenNovel to only use specific providers rather than disabling them one by one.
