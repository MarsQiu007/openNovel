...
Create `.opennovel/commands/test.md`:
...
You can add custom commands through the OpenNovel config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenNovel [config](/docs/config):
...
- Per-project: `.opennovel/commands/`
...
opennovel includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
