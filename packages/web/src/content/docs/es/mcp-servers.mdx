...
Puede agregar herramientas externas a OpenNovel utilizando el _Model Context Protocol_ o MCP. OpenNovel admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OpenNovel Config](https://opennovel.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/opennovel`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
OpenNovel maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OpenNovel:
...
Si el servidor requiere autenticación, OpenNovel le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opennovel mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OpenNovel almacenará los tokens de forma segura en `~/.local/share/opennovel/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OpenNovel, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OpenNovel como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OpenNovel a su cuenta Sentry.
