---
title: Servidor
description: Interactuar con el servidor opennovel a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `opennovel serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opennovel puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `opennovel.local` |
...
Configure `OPENNOVEL_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opennovel`, o configure `OPENNOVEL_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `opennovel serve` como a `opennovel web`.
...
Cuando ejecuta `opennovel`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor opennovel para interactuar con opennovel mediante programación.
:::

Esta arquitectura permite que opennovel admita múltiples clientes y le permite interactuar con opennovel mediante programación.

Puede ejecutar `opennovel serve` para iniciar un servidor independiente. Si tienes el
opennovel TUI ejecutándose, `opennovel serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenNovel [IDE](/docs/ide).
...
El servidor opennovel expone las siguientes API.
