...
Hay dos tipos de agentes en OpenNovel; agentes primarios y subagentes.
...
OpenNovel viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
OpenNovel viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

OpenNovel viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `opennovel.json`:
...
Si no se especifica ninguna temperatura, OpenNovel utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global OpenNovel como para la configuración específica del proyecto.
...
El ID del modelo en su configuración OpenNovel usa el formato `provider/model-id`. Por ejemplo, si usa [OpenNovel Zen](/docs/zen), usaría `opennovel/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `opennovel models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/anomalyco/opennovel).
:::
