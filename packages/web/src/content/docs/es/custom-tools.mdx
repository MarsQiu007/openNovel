...
description: Cree herramientas que LLM pueda llamar en opennovel.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de opennovel como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.opennovel/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/opennovel/tools/`.
...
```ts title=".opennovel/tools/database.ts" {1}
...
```ts title=".opennovel/tools/math.ts"
import { tool } from "@opennovel-ai/plugin"
...
```ts title=".opennovel/tools/bash.ts"
import { tool } from "@opennovel-ai/plugin"
...
```ts title=".opennovel/tools/project.ts" {8}
import { tool } from "@opennovel-ai/plugin"
...
```python title=".opennovel/tools/add.py"
import sys
...
```ts title=".opennovel/tools/python-add.ts" {10}
import { tool } from "@opennovel-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".opennovel/tools/add.py")
