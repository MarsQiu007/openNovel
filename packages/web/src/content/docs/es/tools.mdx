...
Las herramientas permiten que LLM realice acciones en su código base. OpenNovel viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenNovel.
...
:::note
Esta herramienta solo está disponible cuando `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (o `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenNovel o cuando la variable de entorno `OPENNOVEL_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenNovel:

```bash
OPENNOVEL_ENABLE_EXA=1 opennovel
```
