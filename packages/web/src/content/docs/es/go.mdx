...
OpenNovel Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en OpenNovel. Te suscribes a OpenNovel Go y
...
OpenNovel Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

OpenNovel Go funciona como cualquier otro proveedor en OpenNovel.

1. Inicias sesión en **<a href={console}>OpenNovel Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `OpenNovel Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a OpenNovel Go.
:::
...
OpenNovel Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de OpenNovel
usa el formato `opennovel-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`opennovel-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con OpenNovel.
