---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenNovel.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenNovel bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenNovel-konfigurasjonen.
...
i `~/.local/share/opennovel/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenNovel
konfig.
...
OpenNovel Zen er en liste over modeller levert av OpenNovel-teamet som har vært
testet og verifisert for å fungere godt med OpenNovel. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenNovel Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenNovel Zen` og gå til [opennovel.ai/auth](https://opennovel.ai/zen).
...
Det fungerer som alle andre leverandører i OpenNovel og er helt valgfritt å bruke.

---

## OpenNovel Go

OpenNovel Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenNovel-teamet som har vært
testet og verifisert for å fungere godt med OpenNovel.

1. Kjør kommandoen `/connect` i TUI, velg `OpenNovel Go`, og gå til [opennovel.ai/auth](https://opennovel.ai/zen).
...
Slik bruker du Amazon Bedrock med OpenNovel:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenNovel:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `opennovel.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i opennovel støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere opennovel til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenNovel skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenNovel-konfigurasjonen.
...
for å lagre token i OpenNovel auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenNovel bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenNovel
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`opennovel.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `opennovel.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenNovel:
...
Slik bruker du Google Vertex AI med OpenNovel:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenNovel.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenNovel, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) plugin for automatisk å logge hver OpenNovel-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenNovel-samtale oppført som en separat økt.
...
Du kan konfigurere OpenNovel for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenNovel for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenNovel for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenNovel. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opennovel) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenNovel:
...
3. Kopier API-nøkkelen for bruk i OpenNovel.
...
6. **Viktig**: Før du bruker skymodeller i OpenNovel, må du hente modellinformasjonen lokalt:
...
OpenNovel Zen er en liste over testede og verifiserte modeller levert av OpenNovel-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenNovel Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenNovel Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenNovel API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenNovel-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenNovel-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenNovel:
...
Du kan også tilpasse modeller gjennom OpenNovel-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenNovel. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `opennovel.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenNovel forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `opennovel auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenNovel-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenNovel-konfigurasjonen.
