...
Verktøy lar LLM utføre handlinger i kodebasen din. OpenNovel kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i OpenNovel.
...
:::note
Dette verktøyet er bare tilgjengelig når `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenNovel-leverandøren eller når miljøvariabelen `OPENNOVEL_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter OpenNovel:
