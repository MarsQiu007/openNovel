...
description: Skriv dine egne programtillegg for å utvide OpenNovel.
---

Plugins lar deg utvide OpenNovel ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenNovel.
...
- `.opennovel/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/opennovel/node_modules/`.
...
1. Global konfigurasjon (`~/.config/opennovel/opennovel.json`)
2. Prosjektkonfigurasjon (`opennovel.json`)
3. Global plugin-katalog (`~/.config/opennovel/plugins/`)
4. Prosjektpluginkatalog (`.opennovel/plugins/`)
...
OpenNovel kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenNovel SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenNovel.
...
:::note
Hvis du bruker OpenNovel-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenNovel fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenNovel:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenNovel kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenNovel sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://opennovel.ai/docs/sdk) for detaljer.
