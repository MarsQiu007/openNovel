---
title: Tjener
description: Samhandle med OpenNovel-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `opennovel serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenNovel-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `opennovel.local` |
...
Sett `OPENNOVEL_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opennovel` som standard, eller sett `OPENNOVEL_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opennovel serve` og `opennovel web`.
...
Når du kjører `opennovel` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenNovel-serveren til å samhandle med OpenNovel programmatisk.
:::

Denne arkitekturen lar OpenNovel støtte flere klienter og lar deg samhandle med OpenNovel programmatisk.

Du kan kjøre `opennovel serve` for å starte en frittstående server. Hvis du har
OpenNovel TUI kjører, vil `opennovel serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenNovel [IDE](/docs/ide) plugins.
...
opennovel-serveren viser følgende APIer.
