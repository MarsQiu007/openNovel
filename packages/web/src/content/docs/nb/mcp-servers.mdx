...
Du kan legge til eksterne verktøy i OpenNovel ved å bruke _Model Context Protocol_ eller MCP. OpenNovel støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenNovel Config](https://opennovel.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="opennovel.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/opennovel`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="opennovel.jsonc" {15}
{
  "$schema": "https://opennovel.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenNovel håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenNovel:
...
Hvis serveren krever autentisering, vil OpenNovel be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opennovel mcp auth <server-name>`.
...
```json title="opennovel.json" {7-11}
{
  "$schema": "https://opennovel.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
opennovel mcp auth my-oauth-server
```
...
```bash
opennovel mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenNovel lagre tokens sikkert i `~/.local/share/opennovel/mcp-auth.json`.
...
opennovel mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenNovel, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenNovel-konfigurasjonen som alle andre verktøy.
...
```json title="opennovel.json" {11, 14-18}
{
  "$schema": "https://opennovel.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
opennovel mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenNovel til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenNovel-samtale oppført som en separat økt.
