...
OpenNovel Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in OpenNovel. You subscribe to OpenNovel Go and
...
OpenNovel Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

OpenNovel Go works like any other provider in OpenNovel.

1. You sign in to **<a href={console}>OpenNovel Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `OpenNovel Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to OpenNovel Go.
:::
...
OpenNovel Go includes the following limits:
...
uses the format `opennovel-go/<model-id>`. For example, for Kimi K3, you would
use `opennovel-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with OpenNovel as well.
