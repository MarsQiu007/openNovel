...
With OpenNovel you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenNovel uses our own `opennovel` theme.
...
OpenNovel comes with several built-in themes.
...
- Want OpenNovel to match their terminal's appearance
...
OpenNovel supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/opennovel/themes/*.json` or `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.opennovel/themes/*.json`
4. **Current working directory** - Defined in `./.opennovel/themes/*.json`
