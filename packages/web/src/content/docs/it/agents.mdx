...
In OpenNovel esistono due tipi di agenti: agenti primari e subagenti.
...
OpenNovel include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenNovel include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

OpenNovel include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di OpenNovel, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `opennovel.json`:
...
- Globale: `~/.config/opennovel/agents/`
- Per progetto: `.opennovel/agents/`
...
Se non specifichi una temperature, OpenNovel usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenNovel sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenNovel usa il formato `provider/model-id`. Per esempio, se stai usando [OpenNovel Zen](/docs/zen), useresti `opennovel/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `opennovel models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/opennovel).
:::
