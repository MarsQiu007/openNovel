...
Le skill degli agenti permettono a OpenNovel di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenNovel cerca in queste posizioni:

- Config di progetto: `.opennovel/skills/<name>/SKILL.md`
- Config globale: `~/.config/opennovel/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenNovel risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.opennovel/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/opennovel/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.opennovel/skills/git-release/SKILL.md` cosi':
...
OpenNovel elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `opennovel.json`:
...
**Per agenti integrati** (in `opennovel.json`):
