...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. OpenNovel include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in OpenNovel.
...
:::note
Questo strumento e' disponibile solo quando `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (oppure `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenNovel o quando la variabile d'ambiente `OPENNOVEL_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di OpenNovel:
