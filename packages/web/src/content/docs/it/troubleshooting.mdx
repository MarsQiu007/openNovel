...
Per diagnosticare problemi con OpenNovel, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\opennovel\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `opennovel --log-level DEBUG`.

---

## Archiviazione

opennovel salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/opennovel/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\opennovel`
...
OpenNovel Desktop esegue in background un server locale di OpenNovel (il sidecar `opennovel-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `OpenNovel` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opennovel/opennovel.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opennovel\opennovel.jsonc`
...
OpenNovel puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/opennovel/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opennovel\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che OpenNovel possa ricostruirla.

1. Chiudi completamente OpenNovel Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/opennovel`
- **Linux**: elimina `~/.cache/opennovel` (oppure esegui `rm -rf ~/.cache/opennovel`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\opennovel`

3. Riavvia OpenNovel Desktop.

---

### Risolvi problemi di connessione al server

OpenNovel Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `opennovel.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OPENNOVEL_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OPENNOVEL_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, OpenNovel Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di OpenNovel.

---

### Notifiche non visualizzate

OpenNovel Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per OpenNovel nelle impostazioni del sistema operativo, e
...
1. Chiudi OpenNovel Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app OpenNovel Desktop):

- `opennovel.settings.dat` (desktop default server URL)
- `opennovel.global.dat` e `opennovel.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con OpenNovel:
...
Per capire a quali modelli hai accesso, esegui `opennovel models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\opennovel`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. opennovel installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\opennovel`

2. Riavvia opennovel per reinstallare i pacchetti provider piu recenti

Questo forzera opennovel a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
opennovel rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
