---
title: Plugin
description: Scrivi plugin per estendere OpenNovel.
---

I plugin ti permettono di estendere OpenNovel agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenNovel.
...
- `.opennovel/plugins/` - plugin a livello progetto
- `~/.config/opennovel/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/opennovel/node_modules/`.
...
1. Config globale (`~/.config/opennovel/opennovel.json`)
2. Config di progetto (`opennovel.json`)
3. Directory plugin globale (`~/.config/opennovel/plugins/`)
4. Directory plugin di progetto (`.opennovel/plugins/`)
...
OpenNovel esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di opennovel per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere opennovel.
...
:::note
Se usi l'app desktop di OpenNovel, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a opennovel di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a opennovel:
...
L'helper `tool` crea uno strumento personalizzato che opennovel può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in opennovel insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://opennovel.ai/docs/sdk) per i dettagli.
