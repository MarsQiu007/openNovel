...
Con OpenNovel puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenNovel usa il tema `opennovel`.
...
OpenNovel include diversi temi integrati.
...
- Vuole che OpenNovel corrisponda all'aspetto del terminale
...
OpenNovel supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/opennovel/themes/*.json` o `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.opennovel/themes/*.json`
4. **Directory di lavoro corrente** - in `./.opennovel/themes/*.json`
