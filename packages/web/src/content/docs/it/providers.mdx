---
title: Provider
description: Usa qualsiasi provider LLM in OpenNovel.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenNovel usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenNovel.
...
OpenNovel Zen e una lista di modelli forniti dal team OpenNovel, testati e
verificati per funzionare bene con OpenNovel. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenNovel Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opennovel e vai su [opennovel.ai/auth](https://opennovel.ai/auth).
...
Funziona come qualsiasi altro provider in OpenNovel ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenNovel:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui opennovel:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `opennovel.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenNovel non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare opennovel per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché opennovel funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di opennovel.
...
salvare il token nello storage di auth di opennovel.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OpenNovel usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenNovel
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `opennovel.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth).

##### Configurazione

Personalizza tramite `opennovel.json`:
...
Per usare il tuo abbonamento GitHub Copilot con opennovel:
...
Per usare Google Vertex AI con OpenNovel:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui opennovel.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da opennovel, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) per loggare automaticamente ogni conversazione di OpenNovel come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenNovel elencata come sessione separata.
...
Puoi configurare opennovel per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare opennovel per usare modelli locali tramite LM Studio.
...
Puoi configurare opennovel per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenNovel. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opennovel) per dettagli.
:::
...
Per usare Ollama Cloud con OpenNovel:
...
3. Copia la chiave API da usare in OpenNovel.
...
6. **Importante**: Prima di usare modelli cloud in OpenNovel, devi scaricare le informazioni del modello localmente:
...
OpenNovel Zen è una lista di modelli testati e verificati forniti dal team OpenNovel. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenNovel Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenNovel Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenNovel.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di opennovel.
...
5. Puoi anche personalizzarli tramite la configurazione di opennovel. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con OpenNovel:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di opennovel. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con opennovel. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `opennovel.json` nella directory del progetto:
...
I campi `limit` permettono a OpenNovel di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `opennovel auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di opennovel e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config opennovel.
