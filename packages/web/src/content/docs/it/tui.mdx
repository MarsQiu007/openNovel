---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenNovel.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenNovel fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenNovel avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenNovel, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenNovel. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenNovel. _Alias_: `/quit`, `/q`
...
Questo è separato da `opennovel.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo OpenNovel attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `opennovel.default`.
