---
title: TUI
description: Using the OpenNovel terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenNovel provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenNovel starts the TUI for the current directory.
...
When using the OpenNovel TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenNovel. Allows you to select from available providers and add their API keys.
...
Exit OpenNovel. _Aliases_: `/quit`, `/q`
...
This is separate from `opennovel.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenNovel waits after the leader key. Defaults to `2000`.
...
Use `OPENNOVEL_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `opennovel.default`.
