...
OpenNovel uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start OpenNovel with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
opennovel --auto
```

You can also use auto mode with [`opennovel run`](/docs/cli#run).

```bash
opennovel run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where OpenNovel was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
OpenNovel permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, OpenNovel starts from permissive defaults:
...
When OpenNovel prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current OpenNovel session)
