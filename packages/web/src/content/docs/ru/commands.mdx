...
Создайте `.opennovel/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию opennovel или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем opennovel [config](/docs/config):
...
- Глобальный: `~/.config/opennovel/commands/`
- Для каждого проекта: `.opennovel/commands/`
...
opennovel включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
