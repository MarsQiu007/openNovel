...
Навыки агента позволяют opennovel обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
opennovel выполняет поиск в следующих местах:

- Конфигурация проекта: `.opennovel/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/opennovel/skills/<name>/SKILL.md`.
...
Для локальных путей проекта opennovel переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.opennovel/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/opennovel/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.opennovel/skills/git-release/SKILL.md` следующим образом:
...
opennovel перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `opennovel.json`:
...
**Для встроенных агентов** (в формате `opennovel.json`):
