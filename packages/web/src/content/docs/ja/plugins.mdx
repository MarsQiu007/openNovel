---
title: プラグイン
description: OpenNovel を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで OpenNovel を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、OpenNovel のデフォルトの動作を変更したりできます。
...
- `.opennovel/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/opennovel/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/opennovel/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/opennovel/opennovel.json`)
2. プロジェクト設定 (`opennovel.json`)
3. グローバルプラグインディレクトリ (`~/.config/opennovel/plugins/`)
4. プロジェクトプラグインディレクトリ (`.opennovel/plugins/`)
...
OpenNovel は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための OpenNovel SDK クライアント。
...
ここでは、OpenNovel を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://opennovel.ai/docs/sdk) を参照してください。
...
OpenNovel が `.env` ファイルを読み取らないようにします。
...
プラグインは OpenNovel にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、OpenNovel が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して OpenNovel で使用できます。
