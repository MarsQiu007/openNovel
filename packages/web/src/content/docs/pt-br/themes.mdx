...
Com o opennovel, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o opennovel usa nosso próprio tema `opennovel`.
...
O opennovel vem com vários temas embutidos.
...
- Querem que o opennovel corresponda à aparência do seu terminal
...
O opennovel suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/opennovel/themes/*.json` ou `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.opennovel/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.opennovel/themes/*.json`
