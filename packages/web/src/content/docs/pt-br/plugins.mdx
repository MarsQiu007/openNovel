---
title: Plugins
description: Escreva seus próprios plugins para estender o opennovel.
---

Plugins permitem que você estenda o opennovel conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opennovel.
...
- `.opennovel/plugins/` - Plugins em nível de projeto
- `~/.config/opennovel/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/opennovel/node_modules/`.
...
1. Configuração global (`~/.config/opennovel/opennovel.json`)
2. Configuração do projeto (`opennovel.json`)
3. Diretório de plugins global (`~/.config/opennovel/plugins/`)
4. Diretório de plugins do projeto (`.opennovel/plugins/`)
...
O opennovel executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do opennovel para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o opennovel.
...
:::note
Se você estiver usando o aplicativo desktop opennovel, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o opennovel de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao opennovel:
...
O helper `tool` cria uma ferramenta personalizada que o opennovel pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o opennovel junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://opennovel.ai/docs/sdk) para detalhes.
