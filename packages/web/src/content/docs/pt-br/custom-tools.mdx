...
description: Crie ferramentas que o LLM pode chamar no opennovel.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do opennovel, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.opennovel/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/opennovel/tools/`.
