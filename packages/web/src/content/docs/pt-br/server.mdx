---
title: Servidor
description: Interaja com o servidor opennovel via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `opennovel serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opennovel pode usar.
...
```bash
opennovel serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `opennovel.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`              |

`--cors` pode ser passado várias vezes:

```bash
opennovel serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENNOVEL_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opennovel`, ou defina `OPENNOVEL_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `opennovel serve` quanto ao `opennovel web`.

```bash
OPENNOVEL_SERVER_PASSWORD=your-password opennovel serve
```
...
Quando você executa `opennovel`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor opennovel para interagir com o opennovel programaticamente.
:::

Essa arquitetura permite que o opennovel suporte múltiplos clientes e permite que você interaja com o opennovel programaticamente.

Você pode executar `opennovel serve` para iniciar um servidor autônomo. Se você tiver o TUI do opennovel em execução, `opennovel serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do opennovel [IDE](/docs/ide).
...
O servidor opennovel expõe as seguintes APIs.
