...
As ferramentas permitem que o LLM execute ações em sua base de código. O opennovel vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no opennovel.
...
:::note
Esta ferramenta está disponível apenas quando `OPENNOVEL_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENNOVEL_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opennovel ou quando a variável de ambiente `OPENNOVEL_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o opennovel:
