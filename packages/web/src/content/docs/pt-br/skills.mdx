...
As habilidades do agente permitem que o opennovel descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O opennovel pesquisa nesses locais:

- Configuração do projeto: `.opennovel/skills/<name>/SKILL.md`
- Configuração global: `~/.config/opennovel/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o opennovel sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.opennovel/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/opennovel/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.opennovel/skills/git-release/SKILL.md` assim:
...
O opennovel lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `opennovel.json`:
...
**Para agentes embutidos** (em `opennovel.json`):
