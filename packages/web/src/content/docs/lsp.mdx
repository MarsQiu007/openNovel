---
title: LSP Servers
description: OpenNovel integrates with your LSP servers.
---

OpenNovel can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OpenNovel comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENNOVEL_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and opennovel opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your opennovel config.
...
```json title="opennovel.json" {5-13}
{
  "$schema": "https://opennovel.ai/config.json",
  "lsp": {
    "custom-lsp": {
