---
title: Custom Tools
description: Create tools the LLM can call in opennovel.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside opennovel's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.opennovel/tools/` directory of your project.
- Or globally, by placing them in `~/.config/opennovel/tools/`.
