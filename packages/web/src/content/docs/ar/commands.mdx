...
أنشئ الملف `.opennovel/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenNovel أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenNovel:
...
- على مستوى النظام: `~/.config/opennovel/commands/`
- لكل مشروع: `.opennovel/commands/`
...
يتضمن opennovel عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
