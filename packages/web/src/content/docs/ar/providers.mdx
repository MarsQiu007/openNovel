---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenNovel.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenNovel ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenNovel.
...
في `~/.local/share/opennovel/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenNovel.
...
OpenNovel Zen هي قائمة نماذج يوفّرها فريق OpenNovel وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenNovel. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenNovel Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opennovel، ثم انتقل إلى [opennovel.ai/auth](https://opennovel.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenNovel واستخدامه اختياري بالكامل.

---

## OpenNovel Go

OpenNovel Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenNovel، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenNovel.

1. شغّل الأمر `/connect` في TUI، واختر `OpenNovel Go`، ثم انتقل إلى [opennovel.ai/auth](https://opennovel.ai/zen).
...
لاستخدام Amazon Bedrock مع OpenNovel:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل opennovel:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `opennovel.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenNovel غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين opennovel لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل opennovel بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات opennovel.
...
تخزين الرمز في مساحة تخزين مصادقة opennovel.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenNovel نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenNovel
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`opennovel.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [opennovel-gitlab-auth](https://www.npmjs.com/package/opennovel-gitlab-auth).

##### التهيئة

خصّص عبر `opennovel.json`:
...
لاستخدام اشتراك GitHub Copilot مع opennovel:
...
لاستخدام Google Vertex AI مع OpenNovel:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر opennovel، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opennovel-helicone-session](https://github.com/H2Shami/opennovel-helicone-session) لتسجيل كل محادثة OpenNovel تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenNovel مدرجة كجلسة مستقلة.
...
يمكنك ضبط opennovel لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط opennovel لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط opennovel لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenNovel. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opennovel) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenNovel:
...
3. انسخ مفتاح API لاستخدامه في OpenNovel.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenNovel، يجب سحب معلومات النموذج محليا:
...
OpenNovel Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenNovel. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenNovel Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenNovel Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenNovel.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات opennovel.
...
5. يمكنك أيضا تخصيصها عبر إعدادات opennovel. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع OpenNovel:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات opennovel. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع opennovel. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `opennovel.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenNovel بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `opennovel auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات opennovel وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات opennovel.
