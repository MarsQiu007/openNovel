...
مع OpenNovel يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenNovel سمتنا `opennovel`.
...
يأتي OpenNovel مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenNovel مظهر terminal لديهم
...
يدعم OpenNovel نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/opennovel/themes/*.json` أو `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.opennovel/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.opennovel/themes/*.json`
