---
title: الخادم
description: تفاعل مع خادم opennovel عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `opennovel serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opennovel استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `opennovel.local` |
...
عيّن `OPENNOVEL_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `opennovel`، أو عيّن `OPENNOVEL_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `opennovel serve` و `opennovel web`.
...
عند تشغيل `opennovel` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم opennovel للتفاعل مع opennovel برمجيا.
:::

تتيح هذه البنية لـ opennovel دعم عدة عملاء وتمكّنك من التفاعل مع opennovel برمجيا.

يمكنك تشغيل `opennovel serve` لبدء خادم مستقل. إذا كانت واجهة opennovel في terminal (TUI)
قيد التشغيل، فسيبدأ `opennovel serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenNovel لـ [IDE](/docs/ide).
...
يعرض خادم opennovel واجهات API التالية.
