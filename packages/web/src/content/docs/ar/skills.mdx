...
تتيح مهارات الوكيل لـ OpenNovel اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OpenNovel في هذه المواقع:

- إعدادات المشروع: `.opennovel/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/opennovel/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OpenNovel صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.opennovel/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/opennovel/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.opennovel/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OpenNovel المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `opennovel.json`:
...
**للوكلاء المدمجين** (في `opennovel.json`):
