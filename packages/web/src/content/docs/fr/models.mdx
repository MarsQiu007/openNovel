...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez OpenNovel.
...
Voici plusieurs modèles qui fonctionnent bien avec OpenNovel, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration OpenNovel.

```json title="opennovel.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [OpenNovel Zen](/docs/zen), vous utiliserez `opennovel/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="opennovel.jsonc" {7-12,19-24}
{
  "$schema": "https://opennovel.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="opennovel.jsonc" {6-21}
{
  "$schema": "https://opennovel.ai/config.json",
  "provider": {
    "opennovel": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. OpenNovel est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

OpenNovel est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="opennovel.jsonc" {7-18}
{
  "$schema": "https://opennovel.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque OpenNovel démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration OpenNovel.
