---
title: SDK
description: Client JS de type sécurisé pour le serveur opennovel.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK opennovel JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler opennovel par programme.
...
Créez une instance de opennovel :

```javascript
import { createOpenNovel } from "@opennovel-ai/sdk"

const { client } = await createOpenNovel()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `opennovel.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${opennovel.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de opennovel, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@opennovel-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*opennovel" },
})
