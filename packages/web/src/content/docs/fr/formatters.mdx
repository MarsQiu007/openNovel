---
title: Formateurs
description: OpenNovel utilise des formateurs spécifiques au langage.
---

OpenNovel formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

OpenNovel est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, OpenNovel l'utilisera automatiquement.

---

## Comment ça marche

Lorsque OpenNovel écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration OpenNovel.
...
```json title="opennovel.json" {4-14}
{
  "$schema": "https://opennovel.ai/config.json",
  "formatter": {
    "prettier": {
