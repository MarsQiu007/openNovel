---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenNovel.
---

Les plugins vous permettent d'étendre OpenNovel en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenNovel.
...
- `.opennovel/plugins/` - Plugins au niveau du projet
- `~/.config/opennovel/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/opennovel/node_modules/`.
...
1. Configuration globale (`~/.config/opennovel/opennovel.json`)
2. Configuration du projet (`opennovel.json`)
3. Répertoire global des plugins (`~/.config/opennovel/plugins/`)
4. Répertoire des plugins du projet (`.opennovel/plugins/`)
...
OpenNovel exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK opennovel pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre opennovel.
...
:::note
Si vous utilisez l'application de bureau OpenNovel, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez opennovel de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à opennovel :
...
L'assistant `tool` crée un outil personnalisé que opennovel peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour opennovel aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://opennovel.ai/docs/sdk) pour plus de détails.
