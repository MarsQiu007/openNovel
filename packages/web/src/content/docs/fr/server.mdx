---
title: Serveur
description: Interagissez avec le serveur opennovel via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `opennovel serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opennovel peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `opennovel.local` |
...
Définissez `OPENNOVEL_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `opennovel`, ou définissez `OPENNOVEL_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `opennovel serve` et à `opennovel web`.
...
Lorsque vous exécutez `opennovel`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur opennovel pour interagir avec opennovel par programmation.
:::

Cette architecture permet à opennovel de prendre en charge plusieurs clients et vous permet d'interagir avec opennovel par programme.

Vous pouvez exécuter `opennovel serve` pour démarrer un serveur autonome. Si vous avez le
opennovel TUI en cours d'exécution, `opennovel serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenNovel [IDE](/docs/ide).
...
Le serveur opennovel expose les API suivantes.
