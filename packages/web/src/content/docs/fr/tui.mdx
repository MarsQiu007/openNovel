---
title: TUI
description: Utilisation de l’interface utilisateur du terminal OpenNovel.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenNovel fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de OpenNovel démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le OpenNovel TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à OpenNovel. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez OpenNovel. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `opennovel.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle OpenNovel attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `opennovel.default`.
