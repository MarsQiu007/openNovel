...
Avec OpenNovel, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenNovel utilise notre propre thème `opennovel`.
...
OpenNovel est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenNovel corresponde à l'apparence de leur terminal
...
OpenNovel prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/opennovel/themes/*.json` ou `$XDG_CONFIG_HOME/opennovel/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.opennovel/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.opennovel/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/opennovel/themes
vim ~/.config/opennovel/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .opennovel/themes
vim .opennovel/themes/my-theme.json
```
...
{
  "$schema": "https://opennovel.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
