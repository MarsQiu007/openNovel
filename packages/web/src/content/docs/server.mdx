---
title: Server
description: Interact with opennovel server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `opennovel serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opennovel client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `opennovel.local` |
...
Set `OPENNOVEL_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opennovel`, or set `OPENNOVEL_SERVER_USERNAME` to override it. This applies to both `opennovel serve` and `opennovel web`.
...
When you run `opennovel` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the opennovel server to interact with opennovel programmatically.
:::

This architecture lets opennovel support multiple clients and allows you to interact with opennovel programmatically.

You can run `opennovel serve` to start a standalone server. If you have the
opennovel TUI running, `opennovel serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenNovel [IDE](/docs/ide) plugins.
...
The opennovel server exposes the following APIs.
