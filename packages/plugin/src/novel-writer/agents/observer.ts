...
每章必须生成一条章节摘要：
- action: "create"，data 包含 chapter_id（当前章节 ID）、summary（章节摘要，400-500 字）、key_events（关键事件列表，字符串数组）、char_changes（角色变化列表，字符串数组）
- entity_id 建议格式：summary_<chapterId>

**summary 结构化三要素（必做）**：摘要正文必须明确覆盖以下三要素，缺一不可：
1. **时间点** — 本章处于故事时间线的何时（承接上章的时间流逝，如"紧接上章当夜""三日后清晨"）
2. **主要地点** — 本章场景发生地（多场景时按顺序列出）
3. **角色情绪转折** — 有明显情绪变化的角色，从什么状态因什么原因变成什么状态

字数控制在 400-500 字：200 字以内的摘要无法承载上述三要素，超过 500 字会挤占快照预算。

**key_events 情绪转移条目（有情绪转折时必加）**：除常规事件条目外，为每个发生明显情绪转折的角色追加一条结构化条目，格式严格为：

\`情绪转移:角色名:从X因Y变成Z\`

- X = 转变前情绪状态，Y = 触发原因（章节内事件），Z = 转变后情绪状态
- 示例：\`情绪转移:林昭:从平静因发现师尊遇害变成悲愤\`
- 本章没有明显情绪转折时不必强行生成；X/Z 用具体情绪词（平静/愤怒/恐惧/绝望/坚定等），供下游连续性检查比对
...
2. 所有 content / description 长文本必须是纯文本：禁止 Markdown 标题、加粗、斜体、列表、链接、引用和代码块（如 ##、**、-、1.、[文本](链接)、>、三个反引号）；用 \n\n 按主题分段，每段约 80–220 字，任一段超过 600 字必须继续分段
