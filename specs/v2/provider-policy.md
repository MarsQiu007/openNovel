...
Both `action` and `resource` use opennovel's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.opennovel` files is intentionally deferred until `.opennovel` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether opennovel permits an operation through its managed execution paths.
