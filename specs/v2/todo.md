...
The opennovel server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@opennovel-ai/llm` tool loop after replacing its
...
- plugins get global "opennovel" instance like in that post i showed
- opennovel instance has stuff like `opennovel.session.prompt()` or
  `opennovel.tool.register({...})`
...
- decide whether to preserve deprecated `@opennovel-ai/llm` orchestration exports
