...
Move the canonical OpenNovel terminal application from
`packages/opennovel/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/opennovel ---\
                      > @opennovel-ai/tui -> @opennovel-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/opennovel`,
`packages/cli`, or `@opennovel-ai/core`.

The SDK is the TUI's OpenNovel boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/opennovel`.
- Do not replace private `packages/opennovel` imports with `@opennovel-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@opennovel-ai/tui`.

## Ownership Boundary

### `@opennovel-ai/tui` Owns
...
- OpenNovel domain data displayed by the TUI
...
```text
packages/opennovel/src/cli/cmd/tui
```

Its private dependency on `packages/opennovel` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/opennovel/src/*`.
...
Status: Completed. The private `@opennovel-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@opennovel-ai/tui`.
...
- The package has no dependency on `opennovel`, `@opennovel-ai/cli`, or
  `@opennovel-ai/core`.
...
primitives now live in `@opennovel-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@opennovel-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads OpenNovel globals or process state;
...
Replace process-global OpenNovel state with resolved TUI inputs.
...
`@opennovel-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/opennovel` initially.
...
canonical in `@opennovel-ai/tui`. Configured references resolve through the new
...
adapters rather than OpenNovel domain implementations.

Make the SDK the only OpenNovel domain boundary used by the TUI.
...
- Domain-facing TUI code imports OpenNovel data and operations only from
  `@opennovel-ai/sdk`.
...
`@opennovel-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/opennovel`.
...
- The package has no imports from `packages/opennovel`, `packages/cli`, or
  `@opennovel-ai/core`.
...
public `@opennovel-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/opennovel`.
- Keep the legacy embedded worker and server startup in `packages/opennovel`.
- Change those adapters to load config, create transport inputs, and call the
  public `@opennovel-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/opennovel`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@opennovel-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@opennovel-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for OpenNovel domain state.
...
cd packages/opennovel && bun typecheck
...
rg '@opennovel-ai/core|packages/opennovel|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/opennovel/src packages/opennovel/test
```
...
- Build the current-platform `packages/opennovel` binary.
