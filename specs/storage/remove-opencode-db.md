# Remove `packages/opennovel/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/opennovel/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the opennovel legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/opennovel/src/storage/db.ts` are concentrated in 22 source files:

- `packages/opennovel/src/account/repo.ts`
- `packages/opennovel/src/cli/cmd/db.ts`
- `packages/opennovel/src/cli/cmd/import.ts`
- `packages/opennovel/src/cli/cmd/stats.ts`
- `packages/opennovel/src/control-plane/workspace.ts`
- `packages/opennovel/src/index.ts`
- `packages/opennovel/src/node.ts`
- `packages/opennovel/src/permission/index.ts`
- `packages/opennovel/src/project/project.ts`
- `packages/opennovel/src/server/projectors.ts`
- `packages/opennovel/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/opennovel/src/server/shared/fence.ts`
- `packages/opennovel/src/session/message-v2.ts`
- `packages/opennovel/src/session/projectors.ts`
- `packages/opennovel/src/session/prompt.ts`
- `packages/opennovel/src/session/session.ts`
- `packages/opennovel/src/session/todo.ts`
- `packages/opennovel/src/share/share-next.ts`
- `packages/opennovel/src/storage/db.ts`
- `packages/opennovel/src/sync/index.ts`
- `packages/opennovel/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy opennovel database wrapper; `packages/opennovel/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the opennovel wrapper.
...
Status: Completed. `SyncEvent` and the opennovel projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy opennovel database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy opennovel database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy opennovel database wrapper.
...
All migration groups are complete or superseded. `packages/opennovel/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No opennovel data-migration group remains.

The previous opennovel `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate opennovel data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/opennovel`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/opennovel/src`
- `bun typecheck` from `packages/opennovel`
- Relevant package tests from `packages/opennovel`, not the repo root
