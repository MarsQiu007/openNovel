...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an opennovel storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/opennovel` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No opennovel paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@opennovel-ai/effect-drizzle-sqlite`
...
- OpenNovel-specific path/channel selection stays in `packages/opennovel`.

## OpenNovel Adoption Notes

These are not package requirements, but they matter for the later opennovel adoption PR.

The current `packages/opennovel/src/storage/db.ts` has two non-obvious semantics that the opennovel wrapper must preserve when it consumes this adapter:
...
The opennovel wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving opennovel to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@opennovel-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@opennovel-ai/effect-drizzle-sqlite` as a dependency of `packages/opennovel`.
5. Port `packages/opennovel/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus opennovel-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of opennovel's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then opennovel can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay opennovel-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/opennovel`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not opennovel domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/opennovel` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing opennovel's current database runtime.
