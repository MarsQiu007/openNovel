...
- **执行轮次状态**：轮次创建后进入 `running`；指令成功发送后标记为 `completed`，发送或持久化失败时标记为 `failed`
...
- **packages/schema**：`ChapterAnnotation` 增加 `executionRoundId` 可选字段；新增 `ExecutionRound`、批注快照、创建 / 更新输入 schema
- **packages/protocol**：`novel.ts` 分组新增 `novel.create-execution-round` / `novel.update-execution-round` / `novel.execution-rounds` 端点；`UpdateAnnotationInput` 增加 `executionRoundId` 字段
- **packages/novel-store**：新增 `AnnotationExecutionRoundTable`（含状态与不可变批注快照）；`ChapterAnnotationTable` 新增 `execution_round_id` 可空列；新增轮次创建 / 查询 / 更新函数
