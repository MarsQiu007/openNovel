...
- 展开态平时不渲染可见收纳控件，悬停或聚焦对应面板时在分栏边界外侧淡入虚化收纳条，避免遮挡面板内容。
- 收起态完全隐藏面板，保留低透明度幽灵把手和窄热区；hover/focus 后出现完整展开控件。
...
左右 `aside` 继续承载完整内容并保留 `relative` 定位。左栏控件吸附在右侧边界外侧，右栏控件吸附在左侧边界外侧；控件悬浮在主区侧，但不进入 flex 布局，因此不改变分栏宽度。控件默认 `opacity-0`，在 `group-hover` 或 `focus-within` 时淡入；视觉使用半透明渐变、轻边框、圆角、阴影和 `backdrop-blur`，形成柔和的“虚化收纳条”。
...
收起态的 `aside` 恢复为 `hidden`，不再保留 36px 宽度。body 容器改为 `relative`，在左右边缘渲染宽度约 16px 的独立 overlay 热区。热区常显一个低透明度、窄圆角幽灵把手，作为可发现线索；hover/focus 后隐藏幽灵把手并显示完整展开按钮。
...
- [悬停入口初始可发现性略低于常驻图标坞] → 保留常显幽灵把手，并把热区放在可预测的左右边缘。
