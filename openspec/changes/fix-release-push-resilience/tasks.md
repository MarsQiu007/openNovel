...
- [ ] 3.1 手动触发 Release workflow（patch + dev channel），确认 prepare → build-windows → publish 全链成功，远端 tag 与 draft release 正确创建、build 能基于 tag checkout；验证：`gh run view` 各 job 全绿且 `gh release list` 出现新条目
