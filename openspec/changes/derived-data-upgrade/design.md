...
- 现状队列表没有来源列：需要迁移加列 `source`（text，默认 'manual'；升级任务写 'upgrade'），这是本提案唯一的队列 schema 变更，向后兼容（旧行回填 'manual'）。
- worker 已有单章 observer 重建管线（`handleChapterContent` 一次调用重建该章摘要/引用/段摘要/主轴条目），升级只是批量调度器 + 进度聚合视图，不重写重建逻辑。
...
- 触发前展示：确定性项（免费，N 项）+ AI 项（约 N 章 × 1 次 observer 重建调用，每章调用覆盖摘要三要素、实体引用、段摘要与该章主轴条目），用户确认才执行。
...
1. 升级粒度：**v1 只做整本书**。按卷/范围升级增加进度聚合与部分升级的验收面，留待有真实需求后再开。
2. AI 重建的模型配置：**沿用 observer 既有配置**，不新增用户决策面；升级重建与手动编辑触发的重建输出同一质量标准（摘要三要素、结构化主轴）。
3. 升级入口位置：**工作台内横幅**为唯一触发入口（打开书籍时检测并展示）；书架卡片不做角标，避免对未打开的书产生打扰。触发后入口展示进度直至完成或失败。
