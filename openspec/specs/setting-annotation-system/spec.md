...
让世界观条目具备选区批注、高亮展示、显式执行轮次、受控 AI 修改和结果回溯能力，补齐设定侧的协作修订流程。
...
用户选中世界观条目 content 中的文字后，系统 SHALL 支持创建批注。批注 SHALL 保存所属 world_entry、段落索引、段内起始偏移、结束偏移、引用文本、评论、可选替换建议、来源、状态和时间戳。评论必填；引用文本不能为空；段落索引和偏移量必须在当前 content 范围内。取消创建时 SHALL 不写入数据。
...
- **WHEN** 系统收到段落索引越界、偏移量越界或引用文本为空的批注
- **THEN** 系统拒绝创建并返回可读错误

### Requirement: 批注在设定原文中可视化展示

设定详情 SHALL 按段落渲染 content，并根据批注锚点渲染装饰：open 批注 SHALL 使用醒目下划线，已处理批注 SHALL 使用弱化下划线。用户悬浮或查看列表时 SHALL 能看到评论、替换建议和状态。无批注段落 SHALL 正常纯文本展示。
...
创建批注前，系统 SHALL 检查同一段落内选区是否与现有 open 批注重叠。重叠时系统 MUST 拒绝创建并提示用户编辑现有批注；与非 open 批注重叠时 SHALL 允许创建。

#### Scenario: 与 open 批注重叠

- **WHEN** 新选区与同一段落已有 open 批注范围交叉
- **THEN** 系统阻止创建并提示编辑现有批注

#### Scenario: 与已处理批注重叠

- **WHEN** 新选区只与 resolved、wontfix 或 applied 批注重叠
- **THEN** 系统允许创建新批注
