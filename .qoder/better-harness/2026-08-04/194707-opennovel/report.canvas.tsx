...
function mergeCanvasObjects(host, detail) {
  ...
  for (const [key, value] of Object.entries(detail)) {
    merged[key] =
      value && typeof value === "object" && !Array.isArray(value) ? mergeCanvasObjects(host[key], value) : value
  }
  return merged
}
        ...
        >
          <Stack gap={4} style={{ maxHeight: 240, overflowY: "auto", paddingRight: 4 }}>
            {remaining.map((path, index) => (
              <Text key={`${path}-${index}`} size="sm" tone="tertiary" style={{ overflowWrap: "anywhere" }}>
                {path}
              </Text>
            ))}
          </Stack>
...
function visibleUsageSeries(series, offset, limit = 5) {
  ...
  const primary = named.slice(0, limit).map((row) => ({
  ...
  if (remainder.length > 0) {
    ...
    primary.push({
      ...
      data: Array.from({ length }, (_, index) =>
        remainder.reduce((sum, row) => sum + Number(list(row?.daily).slice(offset)[index] ?? 0), 0),
      ),
    })
  }
  return primary
...
function practiceSurfaceGlyph(surface) {
  return (
    { Rules: "R", Skills: "S", "Custom Agents": "A", Hooks: "H", MCP: "M" }[surface] ??
...
function TaskLoopReportHeader({ findings }) {
  ...
  return (
    <Stack gap={8}>
      <H1>{projectName()}</H1>
      {overview ? (
        <Text tone="secondary" style={{ ...taskLoopReaderCopyStyle, lineHeight: 1.5 }}>
          {overview}
        </Text>
      ) : null}
...
function practiceSourceDetail(row) {
  ...
  const pathListStyle =
    remaining.length > 8 ? { maxHeight: 220, overflowY: "auto", paddingRight: 4 } : { paddingRight: 4 }
  return (
    <CollapsibleSection
      size="sm"
      defaultOpen={false}
...
function TaskLoopActivityHeatmap({ activity }) {
  ...
  if (!matrix)
    return (
      <Text tone="secondary">
        {taskLoopCopy("No dated session activity was observed.", "没有观察到带日期的会话活动。")}
      </Text>
    )
  return (
    <Stack gap={6}>
...
function UsageStatRow({ label, value }) {
  ...
  return (
    <Row justify="space-between" align="start" gap={12} style={{ minWidth: 0 }}>
      ...
      <Text size="sm" weight="semibold" style={{ minWidth: 0, overflowWrap: "anywhere", textAlign: "right" }}>
        {value}
      </Text>
    </Row>
  )
...
function TaskLoopModelUsageTable({ rows }) {
  ...
  return (
    <Stack gap={7}>
      ...
      <Table columns={taskLoopModelUsageColumns()} rows={rows} rowKey={(row) => row.model} density="compact" />
    </Stack>
  )
}
...
function TaskLoopLongSessionReview({ usage }) {
  ...
  const longestActiveMinutes =
    usage.longSessions?.longestActiveMinutes ?? Math.max(...samples.map((sample) => Number(sample.activeMinutes ?? 0)))
  return (
    <Stack gap={14}>
    ...
    <Stack gap={12}>
      ...
      <Grid columns="repeat(auto-fit, minmax(min(100%, 260px), 1fr))" gap={24} align="start">
        <Stack gap={9}>
          ...
          {activity ? (
            <UsageStatRow label={taskLoopCopy("Skills used", "Skill 使用")} value={formatUsageNumber(skillUses)} />
          ) : null}
          {longSessions ? (
            <UsageStatRow
              label={taskLoopCopy("Estimated active-long sessions", "估算活跃长会话")}
        ...
        <Grid columns="repeat(auto-fit, minmax(min(100%, 240px), 1fr))" gap={20} align="start">
          ...
          {accounting ? (
            <Stack gap={7}>
              ...
              />
              <UsageStatRow
                label={taskLoopCopy("Model-attributed responses", "已归属模型响应")}
                value={formatUsageNumber(accounting.modelAttributedResponseCount)}
              />
              <UsageStatRow
              ...
              />
              <UsageStatRow
                label={taskLoopCopy("Exact credits available", "可精确计算 credit")}
                value={accounting.exactCreditsAvailable ? taskLoopCopy("Yes", "是") : taskLoopCopy("No", "否")}
              />
            </Stack>
          ) : null}
        </Grid>
  ...
  return (
    <Card size="sm">
      <CardBody>
        <Stack gap={14}>
          <Stack gap={3}>
            <Text weight="semibold" style={{ fontSize: 16, lineHeight: "24px" }}>
              {title}
            </Text>
...
function taskLoopSessionInsightDetail(row) {
  ...
  return (
    ...
    >
      <Stack gap={7}>
        <Text size="sm" tone="tertiary">
          {taskLoopCopy("Insight ID", "洞察 ID")}: {row.id} · {taskLoopCopy("Status", "状态")}: {row.status ?? "—"} ·{" "}
          {taskLoopCopy("Kind", "类型")}: {row.kind ?? "—"} · {taskLoopCopy("Model", "模型版本")}:{" "}
          {row.modelVersion ?? "—"}
        </Text>
        ...
        {evidenceRefs.length ? (
          ...
          <Text size="sm" tone="tertiary">
            {taskLoopCopy("No raw evidence references recorded.", "未记录原始证据引用。")}
          </Text>
        )}
      </Stack>
    </CollapsibleSection>
...
function TaskLoopSessionInsightsDialog({ entries }) {
  return (
    <Dialog
      trigger={
        <Button variant="ghost">{taskLoopCopy(`View all ${entries.length}`, `查看全部 ${entries.length} 条`)}</Button>
      }
...
function TaskLoopFindingDialog({ row }) {
  ...
  return (
    <Dialog
      trigger={<Button variant="ghost">{taskLoopCopy("View details", "查看详情")}</Button>}
      ...
      <Stack gap={16}>
        ...
        <Stack gap={8} style={taskLoopReaderCopyStyle}>
          ...
          {expectedOutput.length ? (
            ...
            <Text size="sm" tone="tertiary">
              {taskLoopCopy("No expected output was recorded.", "未记录预期结果。")}
            </Text>
          )}
        </Stack>
      </Stack>
...
function TaskLoopEvidenceDetails({ usage, boundary, manifest, selection, learning, coverage }) {
  const hasDetailedBoundary =
    Object.keys(boundary).length > 0 || Object.keys(coverage).length > 0 || Object.keys(learning).length > 0
  return (
    <Stack gap={14}>
      {hasDetailedBoundary ? (
        <Stack gap={7} style={taskLoopReaderCopyStyle}>
          ...
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Session selection", "会话抽样")}: {taskLoopSelectionDetailText(selection)}
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Task evidence", "任务证据")}: {taskLoopTaskEvidenceText(coverage)}
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Delivery outcomes", "交付结果")}: {taskLoopDeliveryOutcomeLabel(boundary)}.
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Learning comparison", "学习循环比较")}: {taskLoopLearningDetailText(learning)}
          </Text>
      ...
      <Grid columns={3} minColumnWidth={260} gap={12} align="start">
        ...
        <UsageStatRow
          ...
          value={Array.isArray(boundary.sourceGaps) ? boundary.sourceGaps.length : "—"}
        />
      </Grid>
      {usage ? (
        <>
...
function TaskLoopEvidenceBoundary({ usage }) {
  ...
  const samplingConfidence = textValue(selection.confidence) || taskLoopCopy("Not supplied", "未提供")
    ...
    <Stack gap={12}>
      ...
      <Grid columns={3} minColumnWidth={180} gap={8} align="stretch">
        ...
        <TaskLoopEvidenceFact
          ...
          tone={Array.isArray(boundary.sourceGaps) && boundary.sourceGaps.length ? "warning" : "neutral"}
        />
        <TaskLoopEvidenceFact
