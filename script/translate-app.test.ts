...
describe("translate app", () => {
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "opennovel/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "opennovel/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "opennovel/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
