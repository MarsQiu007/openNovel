...
import { createOpenNovel } from "@opennovel-ai/sdk"
...
`)
  ...
  const opennovel = await createOpenNovel({ port: 0 })
  ...
  try {
    ...
    const session = await opennovel.client.session.create()
    const result = await opennovel.client.session
      .prompt({
