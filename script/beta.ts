...
const model = "opennovel/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/opennovel")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with opennovel...`)
  ...
  try {
    await $`opennovel run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  opennovel failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with opennovel")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with opennovel...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/opennovel`.",
