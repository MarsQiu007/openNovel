...
const desktopLocales: ReadonlySet<Locale> = new Set(locales)
...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "opennovel/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".opennovel/glossary/zh-cn.md"
  if (locale === "zht") return ".opennovel/glossary/zh-tw.md"
  return `.opennovel/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("OpenNovel did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("OpenNovel returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("OpenNovel session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel OpenNovel runs for 'all' (default: 4)
      --model <provider/id>  OpenNovel model (default: opennovel/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running OpenNovel
...
`)
  ...
  if (failed.length) console.error(`\nOpenNovel failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.OPENNOVEL_DISABLE_PROJECT_CONFIG = "1"
  env.OPENNOVEL_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["opennovel", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENNOVEL_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["opennovel", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.OPENNOVEL_CONFIG
  delete env.OPENNOVEL_CONFIG_DIR
  delete env.OPENNOVEL_CONFIG_CONTENT
  delete env.OPENNOVEL_PERMISSION
  delete env.OPENNOVEL_AUTO_SHARE
  return env
}
