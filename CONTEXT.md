# OpenNovel Session Runtime

OpenNovel sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under OpenNovel's shared tool-output directory to retain complete output that was too large for Session history.
...
**OpenNovel Client**:
The generated Promise and Effect APIs derived from the public `HttpApi`; **Embedded OpenNovel** shares the Effect API through an in-memory `HttpClient` against the same router and handlers.
...
**Embedded OpenNovel**:
A scoped in-process host that structurally extends the **OpenNovel Client**, supplies an in-memory HTTP transport, and exposes additional same-process capabilities directly.
...
**角色图钉**：
角色在世界地图上的位置标记，表达当前叙事焦点位置而非实时位置追踪。
_Avoid_: 头像、pin

**地图草稿**：
尚未确认生效的世界地图候选版本，可由 AI 生成或手动编辑产生。
_Avoid_: 临时地图、预览数据

**派生草稿**：
从正式世界地图复制全部要素与图钉创建的地图草稿，用于在既有布局基础上修改。
_Avoid_: 副本草稿、克隆地图
...
- The **PTY Environment** is a server concern rather than a Core PTY concern. PTY creation merges caller values, then the host overlay, then Core-forced terminal invariants such as `TERM` and `OPENNOVEL_TERMINAL`.
- Networked and **Embedded OpenNovel** use the same **OpenNovel Client** and preserve the full HTTP encoding, routing, middleware, and decoding boundary; only the `HttpClient` transport differs.
...
- Creating **Embedded OpenNovel** is scoped. Closing its owning Scope releases the in-process server resources, database resources, registrations, and fibers.
- **Embedded OpenNovel** exposes shared client capabilities and embedded-only capabilities on one object; consumers do not navigate through a nested `.client` property.
- The beta **OpenNovel Client** currently uses plural consumer-facing capability groups such as `sessions`; whether the stable Session namespace should instead be singular `session` must be settled before stabilization. Internal server identifiers do not implicitly define public client names.
- Server's concrete `HttpApi` is authoritative for shared **OpenNovel Client** capabilities. Codegen compiles its Session group directly; the Effect runtime uses an equivalent Protocol-only projection so generated artifacts remain independent of Core and Server.
...
- `@opennovel-ai/protocol` owns Session endpoint construction and middleware placement. Server supplies concrete middleware keys to produce the authoritative build-time API; the client projection supplies transport-only keys without importing Core or Server at runtime.
...
- Promise and Effect network clients ship from `@opennovel-ai/client` behind isolated root and `/effect` exports. The root has no runtime path to Effect; `/effect` imports only Effect, Schema, and Protocol.
- The Effect-native scoped host belongs to `@opennovel-ai/sdk-next`, which will assume the existing `@opennovel-ai/sdk` name after legacy consumers migrate. Client remains network-only and SDK depends one-way on Client.
...
- A capability intended for both networked and **Embedded OpenNovel** belongs in the authoritative public `HttpApi`; embedded-only same-process capabilities extend **Embedded OpenNovel** separately.
...
- The initial common OpenNovel Client does not expose server-global event aggregation. `events.subscribe()` is bounded to the connected OpenNovel instance or workspace; any future cross-instance administrative stream requires a separately designed API.
...
- The stable `sessions.list(...)` design returns a **Page** in both networked and **Embedded OpenNovel**; embedded execution does not define a separate unbounded array-returning list operation. The beta client currently preserves the existing HTTP `{ data, cursor }` envelope until emitter-level Page projection is implemented.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected OpenNovel instance's default or current location; an explicit value selects a known location. Networked and embedded transports use the same handler semantics.
...
- The **Embedded OpenNovel** Layer delegates to the same scoped creation path; it does not define a second implementation.
...
- Ambient project instruction discovery honors `OPENNOVEL_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
...
- Preserve browser-safe `@opennovel-ai/client` and `@opennovel-ai/client/effect` bundles through import-boundary tests.
