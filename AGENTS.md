...
- If a namespace-style value is needed, import the module's own exported namespace by name, for example `import { Project } from "@opennovel-ai/core/project"`, then reference `Project.ID`.
...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/opennovel`.

## Type Checking

- Always run `bun typecheck` from package directories (e.g., `packages/opennovel`), never `tsc` directly.
...
GitHub Issues 是本仓库的 issue tracker。见 `docs/agents/issue-tracker.md`。

### Triage labels

使用默认 triage 标签词汇表，见 `docs/agents/triage-labels.md`。

### Domain docs

single-context：先读根目录 `CONTEXT.md`，再读相关的 `docs/adr/`。见 `docs/agents/domain.md`。
